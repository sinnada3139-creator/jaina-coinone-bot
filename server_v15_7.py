...
BOT_VERSION = "15.7"
CAUSE_ENGINE = "LIVE-MACRO-RSS-v7"
...
def bing_news_rss(query, limit=6):
    ...
    try:
        ...
        r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.7"},timeout=(2.5,5.0))
        r.raise_for_status()
...
def direct_crypto_feeds(limit=10):
    ...
    def fetch_one(source,url):
        key="feed::"+source
        try:
            r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.7"},timeout=(1.8,3.2))
            r.raise_for_status()
...
def direct_macro_feeds(limit=12):
    """v15.7: 검색엔진과 독립된 실시간 거시/지정학/에너지 RSS 공급망.
    LIVE 응답과 CACHE 복구를 구분해 /cause에서 실제 최신성 확인 가능.
    """
    feeds=[
    ...
    def fetch_one(source,url):
        key="livefeed::"+source
        try:
            r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.7"},timeout=(2.0,4.0))
            r.raise_for_status()
            ...
            rows=_news_cache_get(key,limit,True)
            for x in rows: x["live_state"]="CACHE"
            return rows, False
    out=[]; live_sources=0; failed=0
    with ThreadPoolExecutor(max_workers=len(feeds)) as ex:
        futs=[ex.submit(fetch_one,a,b) for a,b in feeds]
        for f in as_completed(futs):
            try:
                rows,ok=f.result(); out.extend(rows or [])
...
def google_news_rss(query, limit=4, priority=False):
    ...
    try:
        r = SESSION.get(url, headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.7"}, timeout=(2.5,4.5))
        r.raise_for_status()
...
def market_cause_analysis_text(force_direction=None):
    ...
    parts=[f"🌐 【현재 시장 {label} 원인 분석 v15.7】",f"🧠 원인엔진 {CAUSE_ENGINE}",f"BTC {btcprice:,.0f}원 · 1분 {btc1:+.2f}% · 5분 {btc5:+.2f}% · 당일 기준 {btc24:+.2f}%",""]
    ...
    if not selected:
        ...
        parts += [f"🧪 v15.7 진단: GEO {dc['GEO']} · OIL {dc['OIL']} · RATE {dc['RATE']} · RISK {dc['RISK']}"]
        parts += dr
        parts.append("")
