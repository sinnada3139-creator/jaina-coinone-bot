...
CLARITY_OFFICIAL_SOURCES = [
    ...
    ('U.S. Senate Floor','https://www.senate.gov/legislative/LIS/floor_activity/08_07_2026_Senate_Floor.htm'),
]

def _clarity_official_rows():
    """Direct Senate-source fallback so /clarity does not depend only on news/RSS indexing."""
    out=[]
    for source,url in CLARITY_OFFICIAL_SOURCES:
        try:
            r=requests.get(url,timeout=8,headers={'User-Agent':'Mozilla/5.0 JainaCoinMonitor/17.5'})
            if r.status_code != 200: continue
            txt=html.unescape(re.sub(r'<[^>]+>',' ',r.text))
            txt=re.sub(r'\\s+',' ',txt)
            low=txt.lower()
            anchors=['h.r.3633','h.r. 3633','digital asset market clarity act','clarity act']
            pos=next((low.find(a) for a in anchors if low.find(a)>=0),-1)
            if pos < 0: continue
            snippet=txt[max(0,pos-180):pos+520].strip()
            stage,level=_clarity_stage(snippet) or ('NEWS','⚪')
            out.append((stage,level,0.0,{'title':snippet[:420],'source':source,'link':url,'official':True}))
        except Exception as e:
...
def clarity_radar_rows(limit=8):
    rows=[]; seen=set()
    # Official U.S. Senate pages first; news feeds remain secondary corroboration.
    for row in _clarity_official_rows():
        key=(row[3].get("source","")+row[3].get("title","")).lower()[:220]
        if key not in seen:
...
def clarity_radar_text():
    parts=['🏛️ 【CLARITY Act 전용 레이더】',
           ...
           '우선순위: U.S. Senate 공식자료 → 주요언론/RSS 교차확인']
    rows=clarity_radar_rows(8)
    if not rows:
        parts.append('• 공식 상원 페이지와 최근 72시간 뉴스에서 신규 상태를 확인하지 못함')
    else:
