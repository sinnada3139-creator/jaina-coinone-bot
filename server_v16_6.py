...
def _v165_google_news(query, limit=8):
    out = []
    try:
        url = 'https://news.google.com/rss/search?q=' + quote_plus(query) + '&hl=ko&gl=KR&ceid=KR:ko'
        ...
        for item in root.findall('.//item')[:limit]:
            ...
            out.append({'title':title, 'link':(item.findtext('link','') or '').strip(),
                        'pubDate':(item.findtext('pubDate','') or '').strip(),
                        'source':((src.text if src is not None else '') or '').strip(),
                        'query':query})
    except Exception:
        pass
    return out
...
def v165_agi_candidates():
    rows, seen = [], set()
    diag = {'queries':len(AGI_WLD_QUERY_PACK),'collected':0,'clean':0,'scored':0}
    for q in AGI_WLD_QUERY_PACK:
        items = _v165_google_news(q, 8)
...
def v165_diag_text():
    ...
    lines = ['🧪 【v16.5 AGI→WLD 뉴스 파이프라인 진단】',
             f"검색쿼리 {d['queries']}개 · 수집 {d['collected']}건 · 정제 {d['clean']}건 · 연관판정 {d['scored']}건"]
    if not rows:
