...
def _official_extract_html(source,url,symbol,limit=12):
    rows=[]
    try:
        r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/17.12"},
...
def official_project_news(symbol,limit=12):
    out=[]
    for source,url in OFFICIAL_PROJECT_SOURCES.get(str(symbol).upper(),[]):
        out.extend(_official_extract_html(source,url,str(symbol).upper(),limit))
...
def official_news_text():
    parts=["🏢 【WLD·KAIA 공식뉴스 레이더 v17.12】",
