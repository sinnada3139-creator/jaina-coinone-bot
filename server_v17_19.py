...
# v17.1: 17분 요약 스케줄 상태/워치독. 배포 후 스레드가 멈추거나 지연돼도 자동 복구.
SUMMARY_HEALTH = {"last_attempt":0.0, "last_success":0.0, "next_due":0.0, "thread_started":0.0}
...
# v17.2: Telegram-configurable staged buy / volume breakout plan.
# Alerts only; this bot never places orders.
BUY_PLAN = {s:{
    "levels": [], "breakout": 0.0, "last_price": 0.0,
    "level_armed": [True, True, True], "breakout_armed": True,
    "last_alert_ts": 0.0
} for s in COINS}
BUYPLAN_COOLDOWN = 17 * 60
BUYPLAN_BREAKOUT_VOL_RATIO = 1.5
...
def load_persistent_state():
    try:
        ...
        for symbol in COINS:
            row = saved_buy_plan.get(symbol, {}) or {}
            try:
                levels = [float(x) for x in list(row.get("levels", []))[:3] if float(x) > 0]
                BUY_PLAN[symbol]["levels"] = levels
                BUY_PLAN[symbol]["breakout"] = float(row.get("breakout", 0.0) or 0.0)
                BUY_PLAN[symbol]["last_price"] = float(row.get("last_price", 0.0) or 0.0)
                BUY_PLAN[symbol]["level_armed"] = list(row.get("level_armed", [True,True,True]))[:3] + [True]*max(0,3-len(list(row.get("level_armed", []))[:3]))
                BUY_PLAN[symbol]["breakout_armed"] = bool(row.get("breakout_armed", True))
                BUY_PLAN[symbol]["last_alert_ts"] = float(row.get("last_alert_ts", 0.0) or 0.0)
            except Exception:
                pass
...
# v16.1: 상승/하락 초기를 중요알람으로 잡고, 같은 흐름이 계속되면 5분마다 재알림
# 상승 초기에는 즉시 뉴스 원인을 분석해 기술적 반등/신규 호재/시장동조를 구분
EARLY_MOVE_LAST = {s:{"ts":0.0,"dir":"","stage":0} for s in COINS}
EARLY_MOVE_REPEAT = 5 * 60
...
def set_buy_plan(symbol, level1, level2, level3, breakout):
    ...
    # 입력 순서를 1·2·3차로 보존한다.
    with LOCK:
        BUY_PLAN[symbol].update({
            "levels": vals, "breakout": bo, "last_price": 0.0,
            "level_armed": [True, True, True], "breakout_armed": True,
            "last_alert_ts": 0.0
        })
    save_persistent_state()
    return symbol, vals, bo

def buy_plan_text(symbol):
    symbol = normalize_symbol(symbol)
    with LOCK:
        row = dict(BUY_PLAN[symbol])
    levels = row.get("levels", [])
    if len(levels) < 3 or safe_float(row.get("breakout")) <= 0:
        return f"📋 {symbol} 매수계획 미설정\n설정: /buyplan {'W' if symbol=='WLD' else 'K'} 585 560 525 680"
    return (f"📋 【{symbol} 예약매수·돌파 계획】\n"
            f"1차 {levels[0]:,.4f}원\n2차 {levels[1]:,.4f}원\n3차 {levels[2]:,.4f}원\n"
            f"거래량 동반 돌파 {safe_float(row.get('breakout')):,.4f}원\n"
            f"돌파 거래량 기준 {BUYPLAN_BREAKOUT_VOL_RATIO:.1f}배 이상\n"
            f"중복알람 쿨다운 17분\n※ 자동주문 없음")

def check_buy_plan_alert(symbol, d):
    """Return alert text only on a fresh downward level cross or volume-confirmed breakout."""
    price = safe_float(d.get("price"))
    vol = safe_float(d.get("vol_ratio"), 1.0)
    if price <= 0:
        return None
    now = time.time()
    with LOCK:
        row = BUY_PLAN[symbol]
        levels = list(row.get("levels", []))[:3]
        breakout = safe_float(row.get("breakout"))
        if len(levels) < 3 or breakout <= 0:
            row["last_price"] = price
            return None
        prev = safe_float(row.get("last_price"))
        row["last_price"] = price
        armed = list(row.get("level_armed", [True,True,True]))
        while len(armed) < 3: armed.append(True)
        # Price recovery above a level rearms that level for a future fresh downward cross.
        for i, level in enumerate(levels):
            if price > level * 1.003:
                armed[i] = True
        if price < breakout * 0.997:
            row["breakout_armed"] = True
        row["level_armed"] = armed
        if prev <= 0:
            return None
        if now - safe_float(row.get("last_alert_ts")) < BUYPLAN_COOLDOWN:
            return None
        for i, level in enumerate(levels):
            if armed[i] and prev > level >= price:
                armed[i] = False
                row["level_armed"] = armed
                row["last_alert_ts"] = now
                return (f"🔵 【{symbol} {i+1}차 예약매수 구간 도달】\n"
                        f"현재가 {price:,.4f}원 · 기준 {level:,.4f}원\n"
                        f"거래량비 {vol:.2f}배\n"
                        f"→ 사전에 정한 {i+1}차 매수구간 확인\n※ 자동주문 없음")
        if bool(row.get("breakout_armed", True)) and prev < breakout <= price and vol >= BUYPLAN_BREAKOUT_VOL_RATIO:
            row["breakout_armed"] = False
            row["last_alert_ts"] = now
            return (f"🚨 【{symbol} 거래량 동반 돌파 중요알람】\n"
                    f"현재가 {price:,.4f}원 · 돌파기준 {breakout:,.4f}원\n"
                    f"거래량비 {vol:.2f}배 (기준 {BUYPLAN_BREAKOUT_VOL_RATIO:.1f}배+)\n"
                    f"→ 가격 돌파 + 거래량 확인\n※ 자동주문 없음")
    return None
...
# ---------- v17.12 OFFICIAL-FIRST PROJECT NEWS RADAR ----------
OFFICIAL_NEWS_LAST={}
...
def _official_norm_title(v):
    return re.sub(r"\s+"," ",html.unescape(str(v or ""))).strip(" \t\r\n-|·")
def _official_key(symbol,title):
    t=re.sub(r"[^a-z0-9가-힣]+"," ",_official_norm_title(title).lower())
    return symbol+":"+re.sub(r"\s+"," ",t).strip()[:180]
def _official_seen(symbol,title,mark=False):
    key=_official_key(symbol,title); now=time.time()
    with OFFICIAL_NEWS_LOCK:
        for k,ts in list(OFFICIAL_NEWS_LAST.items()):
            if now-safe_float(ts)>OFFICIAL_NEWS_REPEAT_TTL: OFFICIAL_NEWS_LAST.pop(k,None)
        old=safe_float(OFFICIAL_NEWS_LAST.get(key))
        if mark: OFFICIAL_NEWS_LAST[key]=now
    return bool(old and now-old<OFFICIAL_NEWS_REPEAT_TTL)
def _official_date_from_html(body):
    """v17.18: robust official article timestamp parser (JSON-LD/meta/time/visible date)."""
    body=str(body or "")
    pats=(
      r'"(?:datePublished|dateModified|dateCreated)"\s*:\s*"([^"]+)"',
      r'<meta[^>]+(?:property|name)=["\'](?:article:published_time|article:modified_time|date|publish_date|datePublished)["\'][^>]+content=["\']([^"\']+)',
      r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+(?:property|name)=["\'](?:article:published_time|article:modified_time|date|publish_date|datePublished)["\']',
      r'<time[^>]+datetime=["\']([^"\']+)["\']',
    )
    for pat in pats:
        m=re.search(pat,body,re.I|re.S)
        if m and _parse_pub_ts(html.unescape(m.group(1)).strip()): return html.unescape(m.group(1)).strip()
    # Official pages sometimes render a human-readable date without metadata.
    txt=_official_norm_title(re.sub(r'<[^>]+>',' ',body))[:12000]
    month=r'(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
    for pat in (rf'\b{month}\s+\d{{1,2}},\s+20\d{{2}}\b', rf'\b\d{{1,2}}\s+{month}\s+20\d{{2}}\b'):
...
def _official_listing_rows(source,url,symbol,raw,limit=24):
    """v17.18: source-specific article-link fallback independent of menu label keywords."""
    out=[]
    host=(urlparse(url).netloc or '').lower()
    for href,label in re.findall(r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>(.*?)</a>',raw,re.I|re.S):
        title=_official_norm_title(re.sub(r'<[^>]+>',' ',label))
        full=urljoin(url,html.unescape(href).strip())
        path=(urlparse(full).path or '').lower()
        is_article=(symbol=='WLD' and '/blog/' in path and path.rstrip('/') not in ('/blog','/blog/announcements')) or \
                   (symbol=='KAIA' and host.endswith('blog.kaia.io') and path.count('/')>=1 and '/tag/' not in path)
        if not is_article or len(title)<12: continue
        out.append({'title':title,'link':full,'source':source,'pubdate':'','official':True,'project':symbol,'official_rank':0})
    return _dedupe_news(out)[:limit]

def _official_extract_html(source,url,symbol,limit=12):
    rows=[]
    try:
        r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/17.19"},
        ...
        rows.extend(_official_listing_rows(source,url,symbol,raw,max(limit*2,24)))
        for block in re.findall(r'<script[^>]+type=["\']application/ld\+json["\'][^>]*>(.*?)</script>',raw,re.I|re.S):
            try:
                obj=json.loads(html.unescape(block)); objs=obj if isinstance(obj,list) else [obj]
                for z in objs:
                    if not isinstance(z,dict): continue
                    cand=z.get("@graph",[]) if isinstance(z.get("@graph"),list) else [z]
                    for it in cand:
                        if not isinstance(it,dict): continue
                        title=_official_norm_title(it.get("headline") or it.get("name"))
                        if len(title)<12: continue
                        rows.append({"title":title,"link":str(it.get("url") or url),
                          "source":source,"pubdate":str(it.get("datePublished") or it.get("dateModified") or ""),
        ...
        if len(rows)<3:
            for href,label in re.findall(r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>(.*?)</a>',raw,re.I|re.S):
                ...
                low=title.lower()
                relevant=(any(k in low for k in ("world","world id","worldcoin","world chain","orb","human","verification","money"))
                          if symbol=="WLD" else
                          any(k in low for k in ("kaia","stablecoin","token","governance","mainnet","ecosystem","foundation","burn")))
                if not relevant: continue
                if href.startswith("/"):
                    m=re.match(r"^(https?://[^/]+)",url); href=(m.group(1) if m else "")+href
    ...
    enriched=[]
    for it in _dedupe_news(rows)[:max(limit,12)]:
        link=urljoin(url,str(it.get("link") or ""))
        it["link"]=link
        if not str(it.get("pubdate") or "").strip() and link and link.rstrip("/") != url.rstrip("/"):
            try:
                d=SESSION.get(link,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/17.19"},timeout=(2.5,5.5),allow_redirects=True)
                if d.ok:
                    body=d.text or ""
                    parsed_date=_official_date_from_html(body)
                    if parsed_date: it["pubdate"]=parsed_date
                    hm=re.search(r'<h1[^>]*>(.*?)</h1>',body,re.I|re.S)
                    if hm:
                        h=_official_norm_title(re.sub(r"<[^>]+>"," ",hm.group(1)))
                        if len(h)>=12: it["title"]=h
            except Exception:
                pass
        enriched.append(it)
    return _dedupe_news(enriched)[:limit]
def official_project_news(symbol,limit=12):
    """v17.13: official sources fetched concurrently so one slow site cannot block all sources."""
    symbol=str(symbol or "").upper()
    sources=list(OFFICIAL_PROJECT_SOURCES.get(symbol,[]))
    out=[]
    if not sources: return out
    with ThreadPoolExecutor(max_workers=min(3,len(sources))) as ex:
        futs=[ex.submit(_official_extract_html,source,url,symbol,limit) for source,url in sources]
        try:
            ...
            pass
    out=_dedupe_news(out); out.sort(key=lambda x:_news_age_hours(x))
    NEWS_HEALTH["official_"+symbol+"_rows"]=len(out)
    NEWS_HEALTH["official_"+symbol+"_dated"]=sum(1 for x in out if _parse_pub_ts(x.get("pubdate")))
    return out[:limit]
def _official_market_tone(title):
    low=(title or "").lower()
    pos=("launch","partnership","integrat","release","upgrade","mainnet","adoption","expansion","approved","reward","burn","소각","출시","통합","파트너","확장","승인")
    neg=("incident","exploit","hack","suspend","sunset","delay","outage","vulnerability","중단","해킹","취약","연기","종료")
    p=sum(k in low for k in pos); n=sum(k in low for k in neg)
    return "🟢 호재후보" if p>n else ("🔴 위험/악재후보" if n>p else "⚪ 공식업데이트")
def _official_is_real_news(it):
    """v17.15: remove homepage/menu/FAQ/category rows from manual official radar."""
    title=_official_norm_title(it.get("title",""))
    link=str(it.get("link") or "").lower()
    low=title.lower()
    if not title or len(title)<18: return False
    menu=("faq","frequently asked","privacy","terms","careers","enterprise",
          "the real human network","announcements","media center","contact",
          "documentation","developers","solutions")
    if any(low==x or low.endswith(" - "+x) for x in menu): return False
    if low in ("world - the real human network","world for enterprise","announcements"): return False
    # Bare home/category URLs are navigation, not an article.
    bare=("https://world.org","https://world.org/","https://blog.kaia.io/",
          "https://blog.kaia.io/tag/announcements/","https://www.kaia.io/media-center")
...
def _official_recent_rows(symbol,limit=8,max_age_h=36.0):
    ...
    for it in official_project_news(symbol,max(limit*2,12)):
        ...
        # Breaking/manual radar must have a parsable timestamp and be inside freshness window.
        if age>max_age_h: continue
        if not _official_is_real_news(it): continue
        rows.append(it)
    return rows[:limit]

def official_news_text():
    parts=["🏢 【WLD·KAIA 공식뉴스 레이더 v17.18】",
    ...
    for symbol in ("WLD","KAIA"):
        ...
        for it in rows[:4]:
            ...
            parts.append(f"{_official_market_tone(it.get('title',''))} · {age_txt}\n{it.get('title','')}\n{it.get('source','공식')} · {it.get('link','')}")
    parts.append("\n※ 공식 게시물이라는 이유만으로 가격상승 원인으로 단정하지 않음 · 자동주문 없음")
    return "\n".join(parts)
def _official_command_worker(cid):
    try:
        send_long(official_news_text(),cid)
    except Exception as e:
        send_long("⚠️ 공식뉴스 조회 일부 지연/실패\n기존 가격·원인·이벤트 감시는 계속 작동합니다.\n"
                  "잠시 후 /official 을 다시 실행해 주세요.",cid)
        print("[OfficialCommand]",type(e).__name__,str(e)[:80],flush=True)

def official_command_async(cid):
    """v17.13: Telegram polling thread never waits for World/Kaia web requests."""
    try:
        send_long("🔎 WLD·KAIA 공식뉴스 조회 시작\n공식 소스를 백그라운드에서 확인 중입니다.",cid)
    except Exception:
        pass
    threading.Thread(target=_official_command_worker,args=(cid,),daemon=True).start()

def official_news_loop():
    baseline=True
    while True:
        try:
            for symbol in ("WLD","KAIA"):
                for it in official_project_news(symbol,12):
                    age=_news_age_hours(it); title=it.get("title","")
                    if age>OFFICIAL_NEWS_MAX_AGE_H: continue
                    if not _official_is_real_news(it): continue
                    if _official_seen(symbol,title,mark=baseline): continue
                    if baseline: _official_seen(symbol,title,mark=True); continue
                    _official_seen(symbol,title,mark=True)
                    if CHAT_ID:
                        send_long(f"🚨 【{symbol} 공식뉴스 우선 알람】\n{_official_market_tone(title)} · 공식 소스 직접 확인\n"
                                  f"{title}\n{it.get('source','공식')} · {it.get('link','')}\n"
                                  "→ 중요알람/원인 레이더에서 최우선 근거로 확인\n"
                                  "※ 공식뉴스 ≠ 가격 인과 확정 · 자동주문 없음",CHAT_ID)
            baseline=False
        except Exception as e:
            print("[OfficialNews]",type(e).__name__,str(e)[:80],flush=True); baseline=False
        time.sleep(OFFICIAL_NEWS_POLL)
# ---------- /v17.12 ----------
        ...
        try:
            ...
            for s,tick in ticks.items():
                if s not in COINS:
                    continue
                d=strategy(s,tick)
                plan_alert = check_buy_plan_alert(s, d)
                if plan_alert and CHAT_ID:
                    send(plan_alert, CHAT_ID)
                    save_persistent_state()
                # v14.6: 순간/누적 급변뿐 아니라 지속 하락+15분/4시간 추세 붕괴도 중요 원인알람
                early = CHAT_ID and early_move_triggered(s,d)
                if early:
                    send(early_move_alert_text(s,d),CHAT_ID)
                    # v16.1: 급등 임계치(1분 3%/5분 5%)를 기다리지 않고 상승 초기부터 원인 분석.
                    # 하락 초기에도 동일 엔진을 사용해 방향 전환 원인을 빠르게 확인한다.
                    threading.Thread(target=early_cause_worker,args=(s,dict(d),CHAT_ID),daemon=True).start()
...
def auto_summary_loop():
    """v17.1 robust 17-minute scheduler: fixed monotonic cadence + health timestamps."""
    SUMMARY_HEALTH["thread_started"] = time.time()
    next_due = time.monotonic() + SUMMARY_INTERVAL
    SUMMARY_HEALTH["next_due"] = time.time() + SUMMARY_INTERVAL
    while True:
        try:
            wait = max(0.0, next_due - time.monotonic())
            time.sleep(min(wait, 30.0))
            if time.monotonic() < next_due:
                continue
            SUMMARY_HEALTH["last_attempt"] = time.time()
            if CHAT_ID and send_summary_once(CHAT_ID):
                SUMMARY_HEALTH["last_success"] = time.time()
            # Keep cadence stable even if one send is slow; never burst-send missed intervals.
            now_m = time.monotonic()
            while next_due <= now_m:
                next_due += SUMMARY_INTERVAL
            SUMMARY_HEALTH["next_due"] = time.time() + max(0.0, next_due-now_m)
        except Exception as e:
            print("[SummaryLoop] error", repr(e), flush=True)
            next_due = time.monotonic() + 30.0
            SUMMARY_HEALTH["next_due"] = time.time() + 30.0
            time.sleep(3)

def summary_watchdog_loop():
    """If no summary attempt occurs for >22 minutes, start one replacement scheduler thread."""
    while True:
        try:
            time.sleep(60)
            started = float(SUMMARY_HEALTH.get("thread_started",0) or 0)
            attempted = float(SUMMARY_HEALTH.get("last_attempt",0) or 0)
            base = attempted or started
            if CHAT_ID and base and time.time()-base > 22*60:
                if SUMMARY_THREAD_LOCK.acquire(blocking=False):
                    try:
                        # Reset health before replacement to prevent repeated watchdog spawns.
                        SUMMARY_HEALTH["thread_started"] = time.time()
                        SUMMARY_HEALTH["last_attempt"] = time.time()
                        print("[SummaryWatchdog] restarting 17m scheduler", flush=True)
                        threading.Thread(target=auto_summary_loop,daemon=True,name="summary-recovery").start()
                    finally:
                        SUMMARY_THREAD_LOCK.release()
        except Exception as e:
            print("[SummaryWatchdog] error", repr(e), flush=True)
...
def _news_title_quality(item):
    """v16.4: reject RSS rows whose title is only a publisher/site label or other low-information metadata."""
    ...
    # Google RSS often appends " - publisher"; inspect the headline part separately.
    headline=low
    if src and headline.endswith(" - "+src):
        headline=headline[:-(len(src)+3)].strip()
    tokens=[x for x in headline.replace('·',' ').replace('-',' ').split() if x]
    if len(tokens) < 3:
        return False
    # Reject repeated publisher/site-name metadata such as "스타트업데일리 - 스타트업데일리".
    norm=[x.strip('.,:;()[]') for x in tokens]
    if len(set(norm)) <= 2 and len(norm) >= 3:
        return False
    if src and (headline == src or headline.replace(' ','') == src.replace(' ','')):
        return False
    return True

def _wld_ai_bridge_score(item):
    """AI/AGI -> human verification/digital identity -> World ID/WLD thematic relevance.
    This is an indirect-catalyst score, never proof that AI news directly causes WLD price moves.
    """
    text=((item.get('title') or '')+' '+(item.get('source') or '')).lower()
    ai=('openai','chatgpt','sam altman','agi','artificial general intelligence','ai agent','ai agents','autonomous agent','computer use')
    impact=('launch','release','released','upgrade','breakthrough','capability','capabilities','agent','agents','autonomous','rollout','debut','출시','공개','업그레이드','에이전트','자율')
    identity=('proof of human','proof-of-human','proof of personhood','human verification','human identity','digital identity','humans','bots','bot','인간 인증','인간증명','디지털 신원','사람 인증')
    direct=('worldcoin','world network','world id','world chain','wld')
    score=0
    if any(k in text for k in ai): score += 3
    if any(k in text for k in impact): score += 2
    if any(k in text for k in identity): score += 3
    if any(k in text for k in direct): score += 4
...
def catalyst_search(symbol):
    if symbol=="WLD":
        positive_queries=[
            ...
            '"proof of human" OR "proof of personhood" AI agents digital identity when:7d',
        ]
...
def bing_news_rss(query, limit=6):
    ...
    try:
        ...
        r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/16.4"},timeout=(2.5,5.0))
        r.raise_for_status()
...
def direct_crypto_feeds(limit=10):
    ...
    def fetch_one(source,url):
        key="feed::"+source
        try:
            r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/16.4"},timeout=(1.8,3.2))
            r.raise_for_status()
...
def direct_macro_feeds(limit=12):
    """v15.8: LIVE RSS 전송 실패 원인을 소스별로 진단한다.
    HTTP/TIMEOUT/CONNECT/XML/EMPTY를 구분하고 /cause에 요약 표시한다.
    """
    feeds=[
    ...
    def fetch_one(source,url):
        ...
        t0=time.time()
        try:
            r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/16.4"},timeout=(2.5,5.0),allow_redirects=True)
            ms=int((time.time()-t0)*1000)
            status=int(getattr(r,"status_code",0) or 0)
            ctype=(r.headers.get("Content-Type") or "")[:45]
            if status >= 400:
                return [],False,f"HTTP {status} {ms}ms"
            try:
                rows=_parse_rss_items(r.content,limit,source)
            except ET.ParseError as e:
                return [],False,f"XML {str(e)[:38]}"
            except Exception as e:
                return [],False,f"PARSE {type(e).__name__}: {str(e)[:32]}"
            if not rows:
                return [],False,f"EMPTY HTTP {status} {ctype or 'no-content-type'}"
            for x in rows: x["live_state"]="LIVE"
            _news_cache_put(key,rows)
            return rows,True,f"OK {status} {ms}ms {len(rows)}건"
        except requests.exceptions.ConnectTimeout:
            return [],False,"CONNECT-TIMEOUT"
        except requests.exceptions.ReadTimeout:
            return [],False,"READ-TIMEOUT"
        except requests.exceptions.SSLError as e:
            return [],False,f"SSL {str(e)[:42]}"
        except requests.exceptions.ConnectionError as e:
            msg=str(e).replace("\n"," ")
            return [],False,f"CONNECT {msg[:48]}"
        except Exception as e:
            return [],False,f"{type(e).__name__}: {str(e)[:48]}"
    out=[]; live_sources=0; failed=0; diag={}
    with ThreadPoolExecutor(max_workers=len(feeds)) as ex:
        futs={ex.submit(fetch_one,a,b):a for a,b in feeds}
        for f in as_completed(futs):
            source=futs[f]
            try:
                rows,ok,detail=f.result(); out.extend(rows or [])
                diag[source]=detail
                if ok: live_sources+=1
                else: failed+=1
            except Exception as e:
                failed+=1; diag[source]=f"FUTURE {type(e).__name__}: {str(e)[:40]}"
    out=_dedupe_news(out)
    NEWS_HEALTH["live_sources"]=live_sources
    NEWS_HEALTH["live_feed_failures"]=failed
    NEWS_HEALTH["live_fresh"]=sum(1 for x in out if x.get("live_state")=="LIVE" and _news_age_hours(x)<=30)
    NEWS_HEALTH["live_feed_diag"]=diag
    if out: _macro_pool_put(out)
    return out
...
def multisource_news(query, limit=7, priority=False):
    ...
    # v15.6: /cause priority 검색은 Bing이 오래된 기사로 개수를 채워도 Google을 반드시 조회한다.
    # Bing RSS는 query의 when:1d를 엄격히 지키지 않아 4~5일 전 기사만 반환할 수 있었다.
    # 일반 호출은 기존처럼 Bing이 충분하면 Google 호출을 생략해 부하를 줄인다.
    bing_fresh=[x for x in out if _news_age_hours(x) <= 30]
    if priority or len(bing_fresh)<max(3,limit//2):
        try:
            out.extend(google_news_rss(query,limit,priority=True if priority else False))
        except Exception as e:
...
def google_news_rss(query, limit=4, priority=False):
    ...
    try:
        r = SESSION.get(url, headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/16.4"}, timeout=(2.5,4.5))
        r.raise_for_status()
...
def _parse_pub_ts(pubdate):
    """v17.17: RFC822 + ISO-8601 공식 사이트 게시시각을 모두 epoch로 변환."""
    v=str(pubdate or "").strip()
    if not v: return 0.0
    try:
        from email.utils import parsedate_to_datetime
...
def _macro_news(direction="DOWN"):
    ...
    first=[]; err1=[]
    try:
        first=_dedupe_news(direct_macro_feeds(18) + direct_crypto_feeds(20) + _macro_pool_get(80))
    except Exception:
        first=_dedupe_news(_macro_pool_get(80))

    # 직접 근거가 충분하지 않을 때만 핵심 검색어 2개를 우선 조회한다.
    # 전체 검색어 폭탄 대신 짧은 1차 보강 후 필요할 때만 심층검색한다.
    fresh0=[x for x in first if _news_age_hours(x)<=30]
    # v15.6: 현재 원인판정에서 발행시각 불명/30h 초과 캐시는 "최신 기사 확보"로 세지 않는다.
    ...
    fresh=[x for x in first if _news_age_hours(x)<=30]
    errors=list(err1)
    if len(fresh) < 8:
        deep, err2=collect_parallel(stage2[:3], limit=8)
        errors.extend(err2)
        fresh=_dedupe_news(fresh + [x for x in deep if _news_age_hours(x)<=30])

    # v15.6 fresh-news failover: 0~24h 근거가 여전히 부족하면 오래된 RSS 개수와 무관하게
    # 현재 지정학→유가→금리/위험회피 경로를 별도 강제검색한다.
    if direction == "DOWN" and len(fresh) < 6:
        fresh_failover_queries=[
            'US Iran airstrikes retaliation Hormuz oil Brent Treasury yields Bitcoin selloff when:1d',
            'Iran strikes oil prices bond yields crypto Bitcoin risk off when:1d',
            '미국 이란 공습 보복 호르무즈 유가 국채금리 비트코인 하락 when:1d',
            'Bitcoin crypto falls Iran strikes oil yields today when:1d',
        ]
        ff, err3=collect_parallel(fresh_failover_queries, limit=10)
        errors.extend(err3)
        ff=[x for x in ff if _news_age_hours(x)<=30]
        fresh=_dedupe_news(ff + fresh)
...
def _event_cluster_cause(items, direction="DOWN"):
    """v15.4: 여러 기사에 흩어진 사건 연쇄를 하나의 원인 클러스터로 결합한다.
    단일 제목이 '이란+유가+BTC하락'을 모두 포함해야 했던 구조적 사각지대를 제거.
    """
    if direction != "DOWN": return None
    buckets={"geo":[],"oil":[],"rates":[],"risk":[]}
    for it in items or []:
        title=(it.get("title") or "").strip(); low=title.lower()
        if not title: continue
        age=_news_age_hours(it)
        # 발행시각 미상도 검색 직후 확보 기사라면 버리지 않는다. 72h 초과가 명확할 때만 제외.
        if age < 999 and age > 72: continue
        q=_source_quality(it.get("source"))
        rec=(it,age,q)
        if any(w in low for w in ("iran","israel","strike","airstrike","missile","attack","retaliat","hormuz","war ","이란","이스라엘","공습","미사일","공격","보복","호르무즈","전쟁")): buckets["geo"].append(rec)
        if any(w in low for w in ("oil","crude","brent","wti","oil price","hormuz","유가","원유","브렌트","호르무즈")): buckets["oil"].append(rec)
        if any(w in low for w in ("treasury","yield","bond yield","rate hike","inflation","hawkish","국채","국채금리","금리 인상","인플레이션","매파")): buckets["rates"].append(rec)
        if any(w in low for w in ("bitcoin","crypto","stocks","nasdaq","risk off","risk-off","selloff","sell-off","slump","falls","drop","비트코인","코인","증시","위험회피","하락","급락")): buckets["risk"].append(rec)
    # 핵심: 서로 다른 기사여도 지정학 + (유가 또는 금리) + 위험자산 반응이 함께 잡히면 사건연쇄로 승격.
    if not buckets["geo"] or not buckets["risk"] or not (buckets["oil"] or buckets["rates"]): return None
    def best(name):
        rows=buckets[name]
        rows.sort(key=lambda r:((_cause_time_weight(r[1]) if r[1] < 999 else 4)+r[2]*2+_direct_event_score(r[0].get("title",""),_macro_category(r[0].get("title","")))),reverse=True)
        return rows[0] if rows else None
    chosen=[best("geo"),best("oil"),best("rates"),best("risk")]
    chosen=[x for x in chosen if x]
    # 동일 기사는 한 번만 표시
    uniq=[]; seen=set()
    for r in chosen:
        k=(r[0].get("title") or "").lower()[:160]
        if k not in seen: seen.add(k); uniq.append(r)
    recent=sum(1 for _,age,_ in uniq if age<=24 or age>=999)
    tier1=sum(1 for _,_,q in uniq if q>=4)
    conf=min(92, 58 + min(18,recent*5) + min(10,tier1*5))
    return {"confidence":conf,"evidence":uniq,"has_oil":bool(buckets["oil"]),"has_rates":bool(buckets["rates"])}

def _cluster_diag(items, limit=12):
    """v15.5: show why evidence did/did not enter GEO/OIL/RATES/RISK buckets."""
    rows=[]; counts={"GEO":0,"OIL":0,"RATE":0,"RISK":0}
    for it in (items or [])[:limit]:
        title=(it.get("title") or "").strip(); low=title.lower(); age=_news_age_hours(it)
        tags=[]
        tests=[
            ("GEO",("iran","israel","strike","airstrike","missile","attack","retaliat","hormuz","war ","이란","이스라엘","공습","미사일","공격","보복","호르무즈","전쟁")),
            ("OIL",("oil","crude","brent","wti","oil price","hormuz","유가","원유","브렌트","호르무즈")),
            ("RATE",("treasury","yield","bond yield","rate hike","inflation","hawkish","국채","국채금리","금리 인상","인플레이션","매파")),
            ("RISK",("bitcoin","crypto","stocks","nasdaq","risk off","risk-off","selloff","sell-off","slump","falls","drop","비트코인","코인","증시","위험회피","하락","급락")),
        ]
        for tag,words in tests:
            if any(w in low for w in words): tags.append(tag); counts[tag]+=1
        age_txt=(f"{age:.1f}h" if age < 999 else "?h")
        why=("/".join(tags) if tags else "NO-TAG")
        if age < 999 and age > 72: why += "/OLD>72h"
        rows.append(f"• {age_txt} [{why}] {title[:110]}")
    return counts,rows
...
def market_cause_analysis_text(force_direction=None):
    ...
    ranked=[]; rejected_opposite=0; rejected_stale_price=0; diag_items=[]
    # v13.8: 뉴스 수집 예외가 /cause 전체를 죽이지 않도록 완전 격리.
    news_collect_error = ""
    try:
        macro_items = _macro_news(direction)
        diag_items=list(macro_items)
    except Exception as e:
    ...
    event_cluster = _event_cluster_cause(macro_items, direction)
    ...
    background_pool=[]
    if not selected and direction=="DOWN":
        emergency_used=True
        emergency_queries=[
            'Bitcoin today falls selloff Fed rates Treasury yields dollar Nasdaq',
            'Bitcoin today long liquidations crypto selloff',
            'Bitcoin today spot ETF outflows flows',
            'Bitcoin today Nasdaq stocks risk off crypto',
        ...
        try: emergency.extend(_macro_pool_get(120))
        except Exception: pass
        for q in emergency_queries:
            try: emergency.extend(_news_cache_get(q,12,True))
            except Exception: pass
        # 네트워크는 최대 4개 병렬. 실패한 검색은 다른 검색과 캐시 결과를 훼손하지 않는다.
        def _em_one(q):
            try:
                items, errs = multisource_news(q, 8, priority=True)
                return items
            except Exception:
                return []
        try:
            with ThreadPoolExecutor(max_workers=4) as ex:
                futs=[ex.submit(_em_one,q) for q in emergency_queries]
                for f in as_completed(futs):
                    try: emergency.extend(f.result() or [])
                    except Exception: pass
        except Exception: pass
        emergency=_dedupe_news(emergency)
        diag_items=_dedupe_news(list(diag_items)+list(emergency))
        # v15.5 root fix: v15.4 built the event cluster BEFORE emergency search,
        # so newly recovered Iran/oil/risk articles could never enter event_cluster.
        # Rebuild the cluster from both normal + emergency evidence immediately.
        if emergency:
            try: _macro_pool_put(emergency)
            except Exception: pass
            recovered_cluster=_event_cluster_cause(_dedupe_news(list(macro_items)+list(emergency)), direction)
            if recovered_cluster:
                event_cluster=recovered_cluster

        eranked=[]
        for it in emergency:
            title=it.get("title",""); cat=_macro_category(title)
            if cat=="시장 수급·기타": continue
            catsupport=_category_support_score(title,cat)
            if catsupport<=0: continue
            age=_news_age_hours(it)
        ...
        if primary_pool:
            selected=[primary_pool[0]]; seen_cats={primary_pool[0][4]}; top_score=primary_pool[0][0]
            for row in primary_pool[1:]:
                if row[4] in seen_cats: continue
                if row[0] >= max(10, top_score*0.50):
                    selected.append(row); seen_cats.add(row[4])
                if len(selected)>=3: break
        elif background_pool:
            # 배경원인만 있을 때는 원인을 단정하지 않고 별도 표시용으로 보관.
            selected=[]
        # NEWS_HEALTH의 '확보 기사 0건'이 복구 성공 후 그대로 표시되지 않도록 보정.
        NEWS_HEALTH["last_ok"]=max(int(NEWS_HEALTH.get("last_ok",0) or 0), len(emergency))
    ...
    parts=[f"🌐 【현재 시장 {label} 원인 분석 v15.9】",f"🧠 원인엔진 {CAUSE_ENGINE}",f"BTC {btcprice:,.0f}원 · 1분 {btc1:+.2f}% · 5분 {btc5:+.2f}% · 당일 기준 {btc24:+.2f}%",""]
    parts += [f"📡 뉴스공급 LIVE {NEWS_HEALTH.get('live_sources',0)}개 소스 · 최신30h {NEWS_HEALTH.get('live_fresh',0)}건 · 실패 {NEWS_HEALTH.get('live_feed_failures',0)}개", ""]
    feed_diag=NEWS_HEALTH.get("live_feed_diag") or {}
    if feed_diag:
        parts.append("🧰 LIVE RSS 전송진단")
        for _src in ("BBC World","BBC Business","Al Jazeera","CNBC World","CNBC Markets","CoinDesk","Cointelegraph","Decrypt"):
            if _src in feed_diag: parts.append(f"• {_src}: {feed_diag[_src]}")
        parts.append("")
    ...
    if emergency_used and selected:
        ...
        parts += ["🛟 긴급검색·영구캐시 복구 성공 — 다만 0~24시간 직접근거는 부족",
                  "🕰️ 24~72시간 기사는 오늘 하락의 직접원인이 아니라 배경원인으로 분리", ""]
    elif emergency_used and not selected:
        parts += ["🛟 지속하락 원인 전용 긴급검색·영구캐시까지 확인했지만 직접 근거 부족", ""]
    if not selected:
        dc,dr=_cluster_diag(diag_items,12)
        parts += [f"🧪 v15.9 진단: GEO {dc['GEO']} · OIL {dc['OIL']} · RATE {dc['RATE']} · RISK {dc['RISK']}"]
        parts += dr
        parts.append("")
    if not selected and event_cluster:
        chain="미국·이란/중동 군사충돌 → 호르무즈·원유 공급위험 → 유가 상승"
        if event_cluster.get("has_rates"): chain += " → 인플레이션·금리/국채금리 부담"
        chain += " → 위험회피 → BTC·알트코인 하락 압력"
        parts += ["🥇 1순위 원인 — 지정학·유가 복합충격", chain, f"🎯 원인 신뢰도 {event_cluster['confidence']}/100", "", "🧩 단일 기사 일치가 아니라 여러 최신 기사의 사건연쇄를 교차검증"]
        for it,age,q in event_cluster["evidence"][:4]:
            age_txt=f"약 {age:.1f}시간 전" if age<999 else "발행시각 확인불가"
            src=f" · {it.get('source')}" if it.get('source') else ""
            parts.append(f"• [{age_txt}] {it.get('title','')}{src}\n{it.get('link','')}")
        parts += ["", "📌 판단: 지정학 사건·유가/금리·위험자산 반응이 서로 다른 기사에 흩어져 있어도 하나의 시장충격 연쇄로 결합합니다."]
    elif selected:
        ...
        parts += ["🔎 0~24시간 우선검색에서 가격 방향과 시간대가 함께 맞는 직접 근거를 확인하지 못했습니다.","🎯 직접 원인 신뢰도 20/100"]
        if background_pool:
            parts.append("🕰️ 배경 원인(24~72시간 · 직접원인으로 단정하지 않음)")
            for row in background_pool[:2]:
                age=row[5]; it=row[6]; cat=row[4]
                parts.append(f"• [{cat} · 약 {age:.1f}시간 전] {it.get('title','')} · {it.get('source','')}\n{it.get('link','')}")
        parts += [f"✅ 반대방향 제목 {rejected_opposite}건 자동 제외",f"✅ 현재 BTC 가격대 불일치 기사 {rejected_stale_price}건 자동 제외","📌 판단: 24시간을 넘긴 뉴스는 배경요인으로만 표시하고 오늘 하락의 직접 원인으로 승격하지 않습니다."]
...
def causetest_text():
    ...
    return (
        "🧪 【v16.1 급변·반등 원인분석 테스트】\n\n"
        "✅ BTC 선행 급락 감지 모듈\n"
        "✅ WLD·KAIA 개별 급변 감지 모듈\n"
        "✅ 상승 초기부터 원인뉴스 즉시 분석\n"
        "✅ 급락 후 기술적 반등 / 신규 프로젝트 호재 / BTC 시장동조 자동 구분\n"
...
# ---------- v17.9 EVENT RESULT -> PRICE REACTION BRIDGE ----------
# A confirmed high-impact event result is kept briefly so the rapid-move engine can
# connect the later price reaction even when the news headline itself is no longer
# returned by the generic RSS search. This fixes the CLARITY 49-50 miss.
EVENT_RESULT_CACHE = {}
EVENT_RESULT_TTL = 6 * 3600
EVENT_RESULT_LOCK = threading.Lock()

def _cache_event_result(event_id, title, direction, source='', link='', confidence=90):
    row={
        'event_id':str(event_id), 'title':str(title), 'direction':str(direction).upper(),
        'source':str(source or ''), 'link':str(link or ''),
        'confidence':int(confidence), 'ts':time.time(),
    }
    with EVENT_RESULT_LOCK:
        EVENT_RESULT_CACHE[str(event_id)] = row
    return row

def _active_event_result(direction):
    now=time.time(); direction=str(direction).upper()
    with EVENT_RESULT_LOCK:
        stale=[k for k,v in EVENT_RESULT_CACHE.items() if now-float(v.get('ts',0))>EVENT_RESULT_TTL]
        for k in stale: EVENT_RESULT_CACHE.pop(k,None)
        rows=[dict(v) for v in EVENT_RESULT_CACHE.values() if v.get('direction')==direction]
    rows.sort(key=lambda x:x.get('ts',0), reverse=True)
    return rows[0] if rows else None

def _clarity_result_direction(title):
    low=(title or '').lower()
    # This is procedural: failed/rejected/not invoked = market-negative event result;
    # invoked/agreed/advanced = market-positive. Never label it final passage.
    neg=('not invoked','cloture rejected','cloture failed','motion to proceed rejected',
         'failed to advance','fails to advance','clarity act failed','49-50','부결','무산')
    pos=('cloture invoked','motion to proceed agreed','agreed to','advanced','advances')
    if any(x in low for x in neg): return 'DOWN'
    if any(x in low for x in pos): return 'UP'
    return None
...
def rapid_cause_text(symbol, d):
    ...
    # v17.9: confirmed scheduled/policy result outranks a generic RSS miss.
    event_hit=_active_event_result(direction)
    if event_hit:
        age_min=max(0,int((time.time()-float(event_hit.get('ts',0)))/60))
        reasons.append(f"확인된 이벤트 결과와 가격 방향 일치: {event_hit.get('title','')} ({age_min}분 전)")
        confidence=max(confidence, int(event_hit.get('confidence',90)))
    ...
    if event_hit:
        verdict="공식/고신뢰 이벤트 결과 직후 같은 방향 가격반응 — 정책 이벤트 연계 가능성이 높음"
    elif btc_aligned and peer_aligned:
    ...
    if event_hit and event_hit.get('link'):
        parts.append("\n🏛️ 확인된 이벤트 결과 근거")
        src=f" · {event_hit.get('source')}" if event_hit.get('source') else ""
        parts.append(f"• {event_hit.get('title','')}{src}\n{event_hit.get('link','')}")
    if matching:
        parts.append("\n📰 시간대가 가까운 관련 기사 후보")
...
def sustained_decline_status(symbol,d):
    ...
    both=short_broken and mid_broken
    # A 전일09시 -4%, B 고점 -12%+추세<=30, C 오늘09시 -3%+고점 -10%
    hit_daily=bool(daily.get("ready") and prev <= -4.0 and both)
    hit_draw=bool(draw <= -12.0 and score <= 30 and both)
    hit_today=bool(daily.get("ready") and today <= -3.0 and draw <= -10.0 and score <= 40 and both)
    hit=hit_daily or hit_draw or hit_today
    reason=f"오늘09시 {today:+.2f}% · 전일09시 {prev:+.2f}% · 고점대비 {draw:+.2f}% · 추세 {score:.0f}/100 · 15분/4시간 {'동시 약세' if both else '확인중'}"
    return hit, reason, (prev if hit_daily else (today if hit_today else draw))

def sustained_decline_triggered(symbol,d):
    hit,reason,move=sustained_decline_status(symbol,d)
    if not hit: return False
    now=time.time(); last=SUSTAINED_LAST[symbol]
    if now-last.get("ts",0)<SUSTAINED_COOLDOWN: return False
    last["ts"]=now
    d["rapid_basis"]="지속하락"; d["rapid_move"]=move
    d["sustained_decline"]=True; d["sustained_reason"]=reason
    return True


def early_move_status(d):
    """v16.0: 급등락 임계치 전에 상승/하락 시작을 3단계로 감지한다."""
    r1=safe_float(d.get("ret1m")); r5=safe_float(d.get("ret5m")); vr=safe_float(d.get("vol_ratio"),1.0)
    # 거래량이 붙으면 더 이른 움직임도 인정. 방향은 더 강한 정규화 변동으로 결정.
    candidates=[]
    if abs(r1)>=0.70 or (abs(r1)>=0.50 and vr>=1.30): candidates.append((abs(r1)/0.70,r1,"1분"))
    if abs(r5)>=1.50 or (abs(r5)>=1.00 and vr>=1.30): candidates.append((abs(r5)/1.50,r5,"5분"))
    if not candidates: return 0,"",0.0,""
    _,move,basis=max(candidates,key=lambda x:x[0])
    direction="UP" if move>0 else "DOWN"
    a1,a5=abs(r1),abs(r5)
    stage=3 if (a1>=3.0 or a5>=5.0) else (2 if (a1>=1.5 or a5>=3.0) else 1)
    return stage,direction,move,basis

def early_move_triggered(symbol,d):
    """단계 상승/방향 전환은 즉시, 같은 단계 지속은 5분마다 중요알람."""
    stage,direction,move,basis=early_move_status(d)
    last=EARLY_MOVE_LAST[symbol]; now=time.time()
    if stage<=0:
        # 완전 진정 뒤에는 다음 초기 움직임을 새 흐름으로 인식
        if now-last.get("ts",0) >= 60:
            last.update({"dir":"","stage":0})
        return False
    changed=(direction!=last.get("dir") or stage!=last.get("stage"))
    repeat=(now-last.get("ts",0) >= EARLY_MOVE_REPEAT)
    if not (changed or repeat): return False
    last.update({"ts":now,"dir":direction,"stage":stage})
    d["early_stage"]=stage; d["early_dir"]=direction; d["early_move"]=move; d["early_basis"]=basis
    return True

def early_move_alert_text(symbol,d):
    direction=d.get("early_dir")
    stage=int(d.get("early_stage",1) or 1)
    icon="📈" if direction=="UP" else "📉"
    name="상승" if direction=="UP" else "하락"
    level={1:"초기",2:"강화",3:"급변"}.get(stage,"초기")
    return (f"🚨 【중요 {name}{level} 알람 — {symbol}】\n"
            f"{icon} {d.get('early_basis','')} {safe_float(d.get('early_move')):+.2f}% · "
            f"1분 {safe_float(d.get('ret1m')):+.2f}% · 5분 {safe_float(d.get('ret5m')):+.2f}%\n"
            f"거래량비 {safe_float(d.get('vol_ratio'),1.0):.2f}배\n"
            f"→ {name} 흐름이 계속되면 5분마다 중요알람으로 재확인합니다.\n"
            f"※ 자동주문 없음")
...
def rebound_cause_text(symbol,d):
    """v16.1 상승 초기에 기술적 반등인지 신규 호재/시장동조인지 빠르게 분류한다."""
    direction=d.get("early_dir") or ("UP" if safe_float(d.get("ret1m"))>=0 else "DOWN")
    if direction != "UP":
        return rapid_cause_text(symbol,d)

    r30=safe_float(d.get("ret30m")); r60=safe_float(d.get("ret1h")); r4h=safe_float(d.get("ret4h"))
    draw=safe_float(d.get("drawdown_pct")); vr=safe_float(d.get("vol_ratio"),1.0)
    move=market_move_snapshot(); btc=move.get("BTC",{}) or {}
    btc_up=((btc.get("ret1") is not None and safe_float(btc.get("ret1"))>=0.6) or
            (btc.get("ret5") is not None and safe_float(btc.get("ret5"))>=1.0))

    items=_rapid_news(symbol,"UP")
    terms=("worldcoin","world network","wld","world chain","world id") if symbol=="WLD" else ("kaia","카이아","line","kakao")
    matches=[]
    for it in items:
        title=(it.get("title") or "")
        sc=_news_direction_score(title,"UP")
        direct=any(x in title.lower() for x in terms)
        if sc>0:
            matches.append((direct,sc,source_weight(it.get("source")),it))
    matches.sort(key=lambda x:(x[0],x[1],x[2]),reverse=True)
    direct_news=[x for x in matches if x[0]]

    was_weak=(r30<=-2.0 or r60<=-3.0 or r4h<=-5.0 or draw<=-7.0)
    if direct_news:
        kind="📰 신규 상승뉴스/프로젝트 호재 반응 가능성"
        confidence=min(95,65 + (10 if vr>=1.5 else 0))
        why=f"{symbol} 직접 관련 상승 재료 후보가 확인됨"
    elif btc_up:
        kind="🌐 시장 동반 반등 가능성"
        confidence=min(90,60 + (10 if vr>=1.5 else 0))
        why="BTC도 같은 시간대 상승 — 시장 전체 위험선호/반등 영향 가능성"
    elif was_weak:
        kind="↩️ 급락 후 기술적 반등 가능성"
        confidence=min(85,58 + (8 if vr<1.5 else 0))
        why=f"직전 누적 약세(30분 {r30:+.2f}% · 1시간 {r60:+.2f}% · 고점대비 {draw:+.2f}%) 뒤 반등이며 직접 호재는 아직 뚜렷하지 않음"
    elif vr>=1.8:
        kind="💧 뉴스 미확인 수급성 상승 가능성"
        confidence=55
        why=f"거래량 {vr:.2f}배 증가가 동반됐지만 직접 뉴스 근거는 아직 부족"
    else:
        kind="❓ 상승 원인 확인 중"
        confidence=35
        why="가격 상승은 확인됐지만 신규 호재·BTC 동조·기술적 반등 중 하나로 확정할 근거가 아직 부족"

    parts=[
        f"🚀 【상승 원인 조기판별 — {symbol}】",
        f"판정 {kind}",
        f"현재가 {safe_float(d.get('price')):,.4f}원 · 1분 {safe_float(d.get('ret1m')):+.2f}% · 5분 {safe_float(d.get('ret5m')):+.2f}%",
        f"30분 {r30:+.2f}% · 1시간 {r60:+.2f}% · 4시간 {r4h:+.2f}% · 거래량 {vr:.2f}배",
        f"🎯 분류 신뢰도 {confidence}/100",
        f"📌 근거: {why}",
    ]
    if matches:
        parts.append("\n📰 같은 방향 최신 기사 후보")
        for direct,_,_,it in matches[:2]:
            tag="프로젝트 직접" if direct else "시장/간접"
            src=f" · {it.get('source')}" if it.get('source') else ""
            parts.append(f"• [{tag}] {it.get('title','')}{src}\n{it.get('link','')}")
    else:
        parts.append("\n📰 현재 가격 시간대와 방향이 맞는 상승 기사 후보 미확인")
    parts.append("※ 신규 뉴스가 뒤늦게 확인되면 다음 중요알람에서 재평가합니다. 자동주문 없음.")
    return "\n".join(parts)


def early_cause_worker(symbol,d,cid):
    """v16.1 초기 움직임 즉시 원인 분석. 뉴스 작업은 직렬화해 중복 폭주를 막는다."""
    try:
        with RAPID_LOCK:
            if (d.get("early_dir") or "") == "UP":
                send_long(rebound_cause_text(symbol,d),cid)
            else:
                send_long(rapid_cause_text(symbol,d),cid)
    except Exception as e:
        print("[EarlyCause] error",symbol,repr(e),flush=True)
...
def _refresh_confirmed_policy_events():
    """v17.10: /cause가 CLARITY 레이더의 최신 RESULT를 직접 읽어 공용 이벤트 상태에 등록."""
    try:
        ...
        for stage,level,age,it in rows:
            ...
            evdir=_clarity_result_direction(title)
            # 최근 결과만 직접 원인 브리지로 승격. 오래된 결과는 배경뉴스로 남김.
            if evdir and safe_float(age,999) <= 8.0:
                _cache_event_result('CLARITY_HR3633', title, evdir,
                                    it.get('source','U.S. Senate'), it.get('link',''), 92)
                return _active_event_result(evdir)
    except Exception as e:
        print('[EventBridge] refresh error',repr(e),flush=True)
    return None

def rapid_cause_report_text():
    """수동 /cause: 현재 변동률을 보여주고, 임계치와 무관하게 시장 원인을 검색한다."""
    # v17.11 FAST PATH: /cause에서 외부 Senate/RSS를 동기 재조회하지 않는다.
    # /clarity 또는 자동 레이더가 저장한 confirmed_event만 즉시 읽는다.
    snap=snapshot(); parts=["🔎 【자이나 현재 급변 원인 레이더 v17.11】"]
    for sym in ("WLD","KAIA"):
        d=snap.get(sym)
        if not d: continue
        ...
        sustained, sustained_reason, _ = sustained_decline_status(sym,d)
        if sustained:
            parts.append("🚨 지속하락/추세붕괴 중요알람 조건 도달")
            parts.append("   " + sustained_reason)
        else:
            parts.append("🛡️ 지속하락 조건 미도달 · " + sustained_reason)
    m=market_move_snapshot().get("BTC",{})
    ...
    forced_dir="DOWN" if sustained_any else None
    event_hit=_active_event_result(forced_dir) if forced_dir else (_active_event_result("DOWN") or _active_event_result("UP"))
    if event_hit:
        age_min=max(0,int((time.time()-float(event_hit.get('ts',0)))/60))
        parts += ["\n🏛️ 【확인된 정책 이벤트 → 가격반응 브리지】",
                  f"🥇 1순위 원인 후보 — {event_hit.get('title','CLARITY Act 절차표결 결과')}",
                  f"🎯 이벤트 신뢰도 {int(event_hit.get('confidence',92))}/100 · 공용 confirmed_event · {age_min}분 전 확인",
                  f"출처: {event_hit.get('source') or 'U.S. Senate'}",
                  event_hit.get('link',''),
                  "※ cloture/motion-to-proceed 절차표결 결과이며 최종 법안 통과/부결과 구분",
                  "※ 가격반응과 시간 일치 시 우선 원인 후보로 표시하며 실제 인과는 추가 확인 필요"]
        parts.append("\n⚡ v17.11 빠른 응답: 확인된 정책 이벤트를 우선 표시했습니다. 외부 뉴스 재조회 때문에 /cause가 멈추지 않습니다.")
    else:
        parts += ["\n🔎 【확인된 정책 이벤트】",
                  "현재 공용 confirmed_event 캐시에 최근 결과가 없습니다.",
                  "→ /clarity 또는 자동 이벤트 레이더가 RESULT를 확인하면 즉시 연결됩니다.",
                  "⚡ /cause는 외부 뉴스 지연과 무관하게 항상 응답하도록 보호됩니다."]
    return "\n".join(parts)
...
def cause_command_worker(cid):
    ...
    NEWS_PRIORITY.set()
    try:
        # v17.11: 수동 /cause는 자동 원인분석의 RAPID_LOCK을 기다리지 않는다.
        # 읽기 전용 fast-path라 장부/주문 상태를 변경하지 않는다.
...
def _project_news_match(symbol,title):
    low=(title or '').lower()
    if symbol=='WLD':
        return any(k in low for k in ('worldcoin','wld','world network','world id','world chain','world money','tools for humanity'))
    return any(k in low for k in ('kaia','klaytn','kaia foundation','kaia chain'))

def news_digest(symbol_filter=None):
    """v17.17: official-first and strict W/K relevance; one source timeout never aborts digest."""
    sf=str(symbol_filter or '').upper().strip()
    if sf in ('W','WORLDCOIN'): sf='WLD'
    if sf in ('K','KAIA'): sf='KAIA'
    wanted=[sf] if sf in ('WLD','KAIA') else ['WLD','KAIA']
    parts=['📰 【자이나 크립토 뉴스 브리핑 v17.17】',market_brief(),'']
    for symbol in wanted:
        parts.append(('🔵 WLD 뉴스' if symbol=='WLD' else '🟣 KAIA 뉴스'))
        official=[]
        try: official=_official_recent_rows(symbol,4,48.0)
        except Exception as e: parts.append('공식소스 일부 지연 ('+type(e).__name__+')')
        if official:
            parts.append('🏢 공식소스 우선')
            for i,it in enumerate(official[:3],1):
                parts.append(f"{_official_market_tone(it.get('title',''))} {i}. {it.get('title','')}\n{it.get('link','')}")
        query=('Worldcoin WLD World Network crypto when:2d' if symbol=='WLD' else 'KAIA blockchain crypto when:2d')
        try:
            items=google_news_rss(query,10)
            items=[it for it in items if _project_news_match(symbol,it.get('title',''))]
            if items: parts.append('🗞 관련 보도')
            for i,it in enumerate(items[:4],1): parts.append(f"{classify_news(it['title'])} {i}. {it['title']}\n{it['link']}")
            if not official and not items: parts.append('새 주요 기사 없음')
        except Exception as e:
            parts.append('관련보도 조회 지연 ('+type(e).__name__+') — 공식/다른 감시는 계속')
        parts.append('')
    parts.append('🚀 더 넓은 호재·예정재료·리스크는 /good 입력')
    parts.append('※ 공식소스 우선 · 프로젝트 직접 관련성 필터 · 자동주문 없음')
    return '\n'.join(parts)
...
def signaltest_messages():
    return [
        (
            ...
            "【테스트】 WLD/KRW\n"
            "신호 🚨 상승초기 감지\n"
            "가정 1분 +0.70% 또는 5분 +1.50%\n"
            "동일 상승흐름 지속 시 5분마다 중요알람 재확인\n\n"
...
CLARITY_QUERIES = [
    '"CLARITY Act" Senate H.R. 3633 when:3d',
    '"Digital Asset Market Clarity Act" Senate when:3d',
    'H.R. 3633 Senate cloture "motion to proceed" when:3d',
    'crypto market structure bill Senate vote amendment ethics stablecoin rewards when:3d',
    'Lummis Boozman Scott CLARITY Act revised text Democrats when:3d',
    'CLARITY Act passes fails advances blocked delayed vote result when:3d',
    'site:reuters.com CLARITY Act crypto Senate when:3d',
]
CLARITY_LAST = {}
CLARITY_COOLDOWN = 45 * 60
CLARITY_ALIASES = ('clarity act','digital asset market clarity','h.r. 3633','h.r.3633','hr 3633','market structure bill','crypto market structure')
CLARITY_CONTEXT = ('crypto','digital asset','senate','lummis','boozman','tim scott','stablecoin','cftc','sec')

def _clarity_stage(title):
    low=(title or '').lower()
    # v17.8: hard bill-ID guard. S.4668 is Protect College Sports Act, never CLARITY.
    foreign_ids = ('s.4668','s. 4668','protect college sports act')
    if any(k in low for k in foreign_ids): return None
    direct=any(k in low for k in CLARITY_ALIASES[:4])
    contextual=any(k in low for k in CLARITY_ALIASES[4:]) and any(k in low for k in CLARITY_CONTEXT)
    if not (direct or contextual): return None
    # v17.7: RESULT requires an actual H.R.3633 outcome, not nearby Senate votes or generic words.
    actual_result=(
        any(k in low for k in ('cloture invoked','cloture rejected','cloture failed','motion to proceed agreed','motion to proceed rejected','h.r.3633 passed','h.r. 3633 passed','clarity act passed','clarity act failed','clarity act rejected','표결 결과','가결','부결'))
        or (('h.r.3633' in low or 'h.r. 3633' in low or 'clarity act' in low) and re.search(r'\b(?:yeas?|nays?|vote|roll call)\b.{0,80}\b\d{1,3}\s*[-–]\s*\d{1,3}\b',low))
    )
    if actual_result: return ('RESULT','🔴')
    if any(k in low for k in ('delay','delayed','postpone','postponed','rescheduled','연기','보류')): return ('CHANGE','🔴')
    if any(k in low for k in ('126','amendment','amended','revised','updated text','new text','final text','deal','agreement','ethics','stablecoin rewards','수정안','합의','최종안')): return ('UPDATE','🟠')
    if any(k in low for k in ('vote','cloture','motion to proceed','scheduled','ripen','hearing','60 votes','표결','예정','청문')): return ('PREVIEW','🟠')
    return ('NEWS','⚪')

CLARITY_OFFICIAL_SOURCES = [
    ('U.S. Senate Cloture','https://www.senate.gov/legislative/cloture/119.htm'),
    ('U.S. Senate Daily Press','https://www.dailypress.senate.gov/'),
    ('U.S. Senate Radio-TV','https://www.radiotv.senate.gov/'),
    ('Senate Banking Committee','https://www.banking.senate.gov/'),
]

def _clarity_clean_text(raw):
...
def _clarity_focus(txt):
    low=txt.lower(); anchors=('h.r.3633','h.r. 3633','digital asset market clarity act','clarity act','regulatory system for digital commodities')
    poses=[low.find(a) for a in anchors if low.find(a)>=0]
    if not poses: return ''
    pos=min(poses)
    # Keep only a tight local window so unrelated nominations/CSS/navigation do not leak into Telegram.
    snippet=txt[max(0,pos-120):pos+430].strip()
    snippet=re.sub(r'(?i)(previous page|next »|senate hearings.*|follow us.*|radio-tv galleries.*)$','',snippet).strip()
    return snippet[:500]
...
def _clarity_source_score(it):
    src=((it.get('source') or '')+' '+(it.get('link') or '')).lower()
    if any(k in src for k in ('senate.gov','congress.gov','govinfo.gov')): return 0
    if 'reuters' in src: return 1
    if any(k in src for k in ('ap news','associated press','bloomberg','cnbc','coindesk','the block')): return 2
    return 3

def clarity_radar_rows(limit=8):
    rows=[]; seen=set(); successful_paths=0
    official=_clarity_official_rows()
    if official: successful_paths += 1
    candidates=list(official)
    for q in CLARITY_QUERIES:
        try:
            items,errs=multisource_news(q,10,priority=True)
            if items: successful_paths += 1
            for it in items:
                title=(it.get('title') or '').strip(); stage=_clarity_stage(title)
                if not title or not stage: continue
                age=_news_age_hours(it)
                if age <= 96: candidates.append((stage[0],stage[1],age,it))
        except Exception as e: print('[ClarityRadar]',type(e).__name__,flush=True)
    # semantic-ish dedupe: one official source/stage and normalized headline duplicates only once.
    for row in candidates:
        stage,level,age,it=row
        title=re.sub(r'\s+',' ',it.get('title','')).strip()
        if not title: continue
        if it.get('official'):
            key='official:'+it.get('source','')+':'+stage
        else:
            key=re.sub(r'[^a-z0-9가-힣]+',' ',title.lower())[:150]
        if key in seen: continue
        seen.add(key); it=dict(it); it['title']=title; rows.append((stage,level,age,it))
    rank={'RESULT':0,'CHANGE':1,'UPDATE':2,'PREVIEW':3,'NEWS':4}
    rows.sort(key=lambda x:(rank.get(x[0],9),_clarity_source_score(x[3]),x[2]))
    NEWS_HEALTH['clarity_paths']=successful_paths
    return rows[:limit]
...
def clarity_radar_text():
    ...
    for _stage,_level,_age,_it in rows:
        if _stage == 'RESULT':
            _title=(_it.get('title') or '').strip()
            _dir=_clarity_result_direction(_title)
            if _dir and (_it.get('official') or safe_float(_age,999) <= 8.0):
                _cache_event_result('CLARITY_HR3633', _title, _dir,
                                    _it.get('source','U.S. Senate'), _it.get('link',''), 92)
                break
    if not rows:
        parts.append('⚠️ 뉴스 수집망 응답 실패/지연 — 신규 기사 없음으로 단정하지 않음' if NEWS_HEALTH.get('clarity_paths',0)==0 else '• 최근 96시간 CLARITY 신규 항목 미확인')
    else:
        labels={'RESULT':'표결결과','CHANGE':'일정변경','UPDATE':'수정·협상','PREVIEW':'표결예고','NEWS':'관련뉴스'}
        for stage,level,age,it in rows:
            age_txt='공식자료' if it.get('official') else (f'{age:.1f}시간 전' if age < 48 else f'{age/24:.1f}일 전')
            parts.append(f'{level} [{labels.get(stage,stage)}] {age_txt}\n{it.get("title","")}'+(f' · {it.get("source","")}' if it.get('source') else '')+f'\n{it.get("link","")}')
    parts += ['※ cloture/motion-to-proceed는 최종 법안 통과와 구분합니다.','※ RESULT는 실제 H.R.3633 표결 결과 확인 때만 표시 · 자동주문 없음']
    return '\n'.join(parts)
            ...
            if CHAT_ID:
                ...
                for stage,level,age,it in clarity_radar_rows(8):
                    ...
                    if stage=='RESULT':
                        evdir=_clarity_result_direction(title)
                        if evdir:
                            _cache_event_result('CLARITY_HR3633', title, evdir, it.get('source','U.S. Senate'), it.get('link',''), 92)
                    send_long(f'{level} 【CLARITY Act 중요알람】\n{label}\n{title}\n{it.get("link","")}\n→ BTC·WLD·KAIA 가격·거래량 후속반응 집중감시\n※ 절차표결과 최종통과를 구분해 판정',CHAT_ID)
        except Exception as e: print('[ClarityRadar] loop error',e,flush=True)
        time.sleep(10*60)
...
LEADING_CATALYST_QUERIES = {
    "WLD": [
        ...
        'OpenAI ChatGPT AGI artificial general intelligence breakthrough model launch release upgrade when:7d',
        'OpenAI ChatGPT AGI 에이전트 출시 업그레이드 인간 인증 디지털 신원 when:7d',
        'OpenAI ChatGPT AI agent agents autonomous computer use when:2d',
        '"Sam Altman" AGI AI agents identity humans bots proof of personhood when:2d',
        '"proof of human" OR "proof of personhood" OR "digital identity" AI agents bots when:2d',
    ],
    "KAIA": [
...
LEADING_INDIRECT = {
    "WLD": ('openai','chatgpt','sam altman','agi','artificial general intelligence','ai agent','ai agents','autonomous agent','proof of human','proof-of-human','proof of personhood','digital identity','human verification','human identity'),
    "KAIA": ('stablecoin','스테이블코인','krw','원화','bank','은행','kakao','카카오','line','라인','rwa','payment','결제'),
}
LEADING_POSITIVE = ('agi','breakthrough','model','release','released','upgrade','upgraded','capability','capabilities','agent','agents','partnership','partner','launch','expand','expansion','adoption','integrat','approval','approve','pass','passed','growth','record','revenue','investment','invest','stablecoin','rwa','제휴','출시','확대','도입','승인','통과','성장','매출','투자')
...
def _leading_grade(symbol,title,source=''):
    ...
    if direct and future:
        return ('A','직접 예정 이벤트')
    if symbol=='WLD':
        bridge=_wld_ai_bridge_score(item)
        # Major OpenAI/AGI releases are allowed into A-grade as a thematic precursor,
        # but the Telegram text explicitly says this is not direct WLD causality.
        if bridge >= 5:
            return ('A','AI/AGI→인간인증 간접 선행호재')
    if indirect and (positive or future):
        return ('A','간접 선행호재 후보')
    return None

def leading_catalyst_rows(symbol,limit=5):
    """v17.17: official project news is first-class S-grade evidence; search news is fallback."""
    rows=[]; seen=set()
    try:
        for it in _official_recent_rows(symbol,max(limit,6),48.0):
            title=(it.get('title') or '').strip()
            if not title: continue
            key=_official_key(symbol,title)
            if key in seen: continue
            seen.add(key)
            tone=_official_market_tone(title)
            if tone.startswith('🔴'):
                continue
            rows.append(('S','직접 공식 프로젝트 업데이트',it))
    except Exception as e:
        print('[LeadingRadarOfficial]',symbol,type(e).__name__,flush=True)
    for q in LEADING_CATALYST_QUERIES[symbol]:
        try:
            for it in google_news_rss(q,5):
                title=(it.get('title') or '').strip()
                if not title: continue
                grade=_leading_grade(symbol,title,it.get('source',''))
                if not grade: continue
                key=title.lower()[:180]
                if key in seen: continue
                seen.add(key); rows.append((grade[0],grade[1],it))
        except Exception as e:
            print('[LeadingRadar]',symbol,type(e).__name__,flush=True)
    rows.sort(key=lambda x: (0 if x[0]=='S' else 1, _news_age_hours(x[2])))
    return rows[:limit]
...
def telegram_loop():
    ...
    while True:
        try:
            ...
            print("[Telegram] getUpdates error",type(e).__name__,str(e)[:80],flush=True)
            time.sleep(3)
            continue
        ...
        for u in r.get("result",[]):
            ...
            if text.startswith("/start") or text.lower()=="start":
                ...
                def _agidiag_worker(chat_id):
                    try:
                        send_long(v165_diag_text(), chat_id)
                    except Exception as e:
                ...
                try:
                    p=text.split()
                    if len(p)==2:
                        send(buy_plan_text(p[1]), cid)
                    elif len(p)==6:
                        symbol, levels, bo = set_buy_plan(p[1],p[2],p[3],p[4],p[5])
                        send("✅ 매수·돌파 계획 저장 완료\n" + buy_plan_text(symbol), cid)
                    else:
...
<div class="w"><h2>자이나 코인원 감시봇 v17.2</h2><small>WLD · KAIA / 준비·실행·황금구간 / 자동주문 없음</small><div id="x"></div></div>
<script>
...
# ===== v16.5 AGI/WLD NEWS PIPELINE DIAGNOSTIC + FALLBACK =====
AGI_WLD_QUERY_PACK = [
    'OpenAI AGI', 'ChatGPT AGI', '"AI agent" OpenAI',
    '"artificial general intelligence" OpenAI',
    '"proof of personhood" AI', '"proof of human" AI',
    '"World ID" AI agent', 'Sam Altman AGI identity',
]

def _v165_clean_title(s):
    s = re.sub(r'\s+', ' ', str(s or '')).strip()
    low = s.lower()
    if not s or len(s) < 12:
        return ''
    if '스타트업데일리 - 스타트업데일리' in low or 'startupdaily - startupdaily' in low:
        return ''
    return s

def _v165_ai_relevance(title):
    t = str(title or '').lower()
    ai = any(k in t for k in ('openai','chatgpt','agi','artificial general intelligence',
                               'ai agent','ai agents','agentic','sam altman'))
    identity = any(k in t for k in ('world id','worldcoin','proof of personhood','proof of human',
                                     'human verification','digital identity','identity'))
    if ai and identity:
        return 82, 'A', 'AI/AGI → 인간인증·디지털신원 → World ID/WLD 연결 후보'
    if ai:
        return 64, 'A', 'AI/AGI 대형이슈 → 인간증명 수요 가능성 점검(간접 선행호재)'
    if identity:
        return 58, 'B', '인간인증·디지털신원 테마 후보'
    return 0, '', ''

def _v167_parse_rss_bytes(content, query, source_name, limit=8):
    out=[]
    try:
        root=ET.fromstring(content)
        for item in root.findall('.//item')[:limit]:
            title=_v165_clean_title(item.findtext('title',''))
            if not title: continue
            ...
            out.append({
                ...
                'link':(item.findtext('link','') or '').strip(),
                'pubDate':(item.findtext('pubDate','') or '').strip(),
                'source':((src.text if src is not None else source_name) or source_name).strip(),
                'query':query, 'collector':source_name
            })
    except Exception:
        pass
    return out

def _v167_fetch(url, timeout=3):
    try:
        r=requests.get(url, timeout=timeout, headers={
            'User-Agent':'Mozilla/5.0 (compatible; JainaCoinMonitor/16.7)',
            'Accept':'application/rss+xml, application/xml, text/xml, */*'
        })
        if r.status_code==200 and r.content:
            return r.content, f"HTTP {r.status_code}"
        return b'', f"HTTP {getattr(r,'status_code','?')}"
    except Exception as e:
        return b'', type(e).__name__

def _v167_google_news(query, limit=8):
    urls=[
      ('GoogleNews-KR','https://news.google.com/rss/search?q='+quote_plus(query)+'&hl=ko&gl=KR&ceid=KR:ko'),
      ('GoogleNews-US','https://news.google.com/rss/search?q='+quote_plus(query)+'&hl=en-US&gl=US&ceid=US:en'),
    ]
    for name,url in urls:
        content,status=_v167_fetch(url)
        rows=_v167_parse_rss_bytes(content,query,name,limit) if content else []
        if rows: return rows,name,status
    return [],'GoogleNews',status if 'status' in locals() else 'no-response'

def _v167_bing_news(query, limit=8):
    """v17.0: Bing 200 응답을 기존 검증 RSS 파서로 우선 해석하고 변형 URL도 시도."""
    urls=[
        'https://www.bing.com/news/search?q='+quote_plus(query)+'&format=rss&mkt=en-US',
        'https://www.bing.com/news/search?q='+quote_plus(query)+'&format=RSS',
    ]
    last='no-response'
    for url in urls:
        content,status=_v167_fetch(url)
        ...
        rows=[]
        try:
            rows=_parse_rss_items(content,limit,'Bing News')
            for x in rows:
                x['pubDate']=x.get('pubdate','')
                x['query']=query
                x['collector']='BingNews'
        except Exception:
            rows=_v167_parse_rss_bytes(content,query,'BingNews',limit)
        if rows:
            return rows,'BingNews',status
    return [],'BingNews',last

def _v170_direct_feed_candidates(limit=30):
    """기존 봇에서 실제 사용 중인 직접 뉴스 피드를 AGI/WLD 후보망으로 재사용."""
    out=[]
    try:
        pool=direct_crypto_feeds(limit)
        out.extend(pool or [])
    except Exception:
        pass
    try:
        pool=direct_macro_feeds(limit)
        out.extend(pool or [])
    except Exception:
        pass
    clean=[]
    ...
    for x in out:
        ...
        title=_v165_clean_title(x.get('title',''))
        if not title: continue
        low=title.lower()
        # AGI/OpenAI/ChatGPT/AI agent + World/identity/biometric/orb 연결 후보까지 넓게 수집
        ai=any(k in low for k in ('openai','chatgpt','agi','artificial general intelligence',
                                  'ai agent','ai agents','agentic ai','sam altman'))
        wid=any(k in low for k in ('worldcoin','world id','world chain','orb','proof of human',
                                   'proof of person','digital identity','biometric'))
        if not (ai or wid): continue
        key=low
        if key in seen: continue
        seen.add(key)
        y=dict(x); y['title']=title; y['collector']='DirectFeeds'
        y['pubDate']=y.get('pubDate') or y.get('pubdate','')
        clean.append(y)
    return clean[:limit]

def _v167_collect(query, limit=8):
    rows,name,status=_v167_google_news(query,limit)
    trace=[(name,len(rows),status)]
    if rows: return rows,trace
    rows2,name2,status2=_v167_bing_news(query,limit)
    trace.append((name2,len(rows2),status2))
    return rows2,trace


def _v169_source_health():
    """Always return a status row, even when every news endpoint times out."""
    probes = [
        ('GoogleNews-KR','https://news.google.com/rss?hl=ko&gl=KR&ceid=KR:ko'),
        ('GoogleNews-US','https://news.google.com/rss?hl=en-US&gl=US&ceid=US:en'),
        ('BingNews','https://www.bing.com/news/search?q=OpenAI&format=rss'),
    ]
    def one(name,url):
        try:
            r=requests.get(url,timeout=2,headers={'User-Agent':'Mozilla/5.0'})
            return name, f"HTTP {r.status_code}", len(r.content or b'')
        except Exception as e:
            return name, type(e).__name__, 0
    out={}
    with ThreadPoolExecutor(max_workers=3) as ex:
        futs=[ex.submit(one,*p) for p in probes]
        try:
            for f in as_completed(futs,timeout=3):
                name,status,size=f.result()
                out[name]={'attempts':1,'items':0,'last':f'{status} · {size}B'}
        except Exception:
            pass
    for name,_ in probes:
        out.setdefault(name,{'attempts':1,'items':0,'last':'TIMEOUT/NO RESULT'})
    return out

def _v169_existing_news_fallback():
    """v17.0: 기존 직접 RSS망을 우선 재사용하고, 그 다음 일반 뉴스 함수를 탐색."""
    candidates=[]
    try:
        candidates.extend(_v170_direct_feed_candidates(30))
    except Exception:
        pass
    if candidates:
        return candidates
    for fn_name in ('fetch_news','get_news','news_items','collect_news','fetch_google_news'):
        fn=globals().get(fn_name)
        if not callable(fn): continue
        for q in ('OpenAI AGI','ChatGPT AGI','AI agent OpenAI','World ID AI'):
            try:
                got=fn(q)
                if isinstance(got,list):
                    for x in got[:8]:
                        if isinstance(x,dict):
                            title=_v165_clean_title(x.get('title',''))
                            if title:
                                y=dict(x); y['title']=title; y['collector']='existing-news'
                                candidates.append(y)
...
def v165_agi_candidates():
    rows,seen=[],set()
    diag={'queries':len(AGI_WLD_QUERY_PACK),'collected':0,'clean':0,'scored':0,'sources':_v169_source_health()}

    # First reuse any collector already proven by /news or the legacy radar.
    legacy=_v169_existing_news_fallback()
    if legacy:
        diag['sources']['DirectFeeds/Fallback']={'attempts':1,'items':len(legacy),'last':'OK'}

    # v16.8: run query collectors concurrently so one dead RSS endpoint cannot
    # block Telegram commands for a long time.
    results=[]
    if legacy:
        results.append(('existing-news',legacy,[('existing-news',len(legacy),'OK')]))
    with ThreadPoolExecutor(max_workers=8) as ex:
        futs={ex.submit(_v167_collect,q,6):q for q in AGI_WLD_QUERY_PACK}
        try:
            for fut in as_completed(futs, timeout=7):
                q=futs[fut]
                try:
                    items,trace=fut.result(timeout=0)
                except Exception as e:
                    items,trace=[],[('collector',0,type(e).__name__)]
                results.append((q,items,trace))
        except Exception:
            pass

    for q,items,trace in results:
        for name,n,status in trace:
            d=diag['sources'].setdefault(name,{'attempts':0,'items':0,'last':status})
            d['attempts']+=1; d['items']+=n
            d['last']=str(d.get('last',''))+' | collect:'+str(status)
        diag['collected']+=len(items)
        for it in items:
            key=it['title'].lower()
            if key in seen: continue
            seen.add(key); diag['clean']+=1
            score,grade,why=_v165_ai_relevance(it['title'])
            if score:
                diag['scored']+=1
                it.update(score=score,grade=grade,why=why)
                rows.append(it)
    rows.sort(key=lambda x:x.get('score',0),reverse=True)
    return rows[:12],diag

def v165_diag_text():
    rows, d = v165_agi_candidates()
    lines = ['🧪 【v17.0 AGI→WLD 다중소스 진단】',
             f"검색쿼리 {d['queries']}개 · 최종수집 {d['collected']}건 · 정제 {d['clean']}건 · 연관판정 {d['scored']}건",
             '',
             '🌐 소스 상태']
    for name,info in d.get('sources',{}).items():
        lines.append(f"• {name}: 시도 {info['attempts']} · 수집 {info['items']} · {info['last']}")
    if not rows:
        lines += ['', '❌ AGI/OpenAI 후보 0건', '→ 아래 소스 상태의 HTTP/TIMEOUT 결과를 기준으로 수집망 장애를 판별합니다.']
    else:
        lines += ['', '🔎 상위 후보']
        for i,it in enumerate(rows[:5],1):
            lines += [f"{i}. [{it['grade']}·{it['score']}/100] {it['title']}", f"   {it['why']}"]
            if it.get('link'): lines.append(it['link'])
    lines += ['', '※ AI/AGI 단독 뉴스는 WLD 직접 호재가 아니라 간접 선행후보로만 분류합니다.']
    return '\n'.join(lines)

_v165_original_handle_update = globals().get('handle_update')
if callable(_v165_original_handle_update):
    def handle_update(u):
        try:
            msg = u.get('message') or u.get('edited_message') or {}
            txt = str(msg.get('text') or '').strip().split()[0].lower()
            cid = msg.get('chat',{}).get('id')
            if txt == '/agidiag' and cid:
                send(v165_diag_text(), cid); return
        except Exception:
            pass
        return _v165_original_handle_update(u)
# ===== END v16.5 =====
...
threading.Thread(target=official_news_loop,daemon=True).start()
...
threading.Thread(target=auto_summary_loop,daemon=True,name="summary-main").start()
threading.Thread(target=summary_watchdog_loop,daemon=True,name="summary-watchdog").start()
