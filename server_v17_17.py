    ...
    for it in _dedupe_news(rows)[:max(limit,12)]:
        ...
        if not str(it.get("pubdate") or "").strip() and link and link.rstrip("/") != url.rstrip("/"):
            try:
                ...
                if d.ok:
                    body=d.text or ""
                    dates=re.findall(r'\"(?:datePublished|dateModified)\"\s*:\s*\"([^\"]+)\"',body,re.I)
                    if not dates:
                        dates=re.findall(r'<meta[^>]+(?:property|name)=["\'](?:article:published_time|date|publish_date)["\'][^>]+content=["\']([^"\']+)',body,re.I)
                    if dates: it["pubdate"]=dates[0]
...
def official_news_text():
    parts=["🏢 【WLD·KAIA 공식뉴스 레이더 v17.16】",
