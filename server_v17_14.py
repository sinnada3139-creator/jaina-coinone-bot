...
def _official_extract_html(source,url,symbol,limit=12):
    rows=[]
    try:
        r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/17.14"},
...
def official_news_text():
    parts=["🏢 【WLD·KAIA 공식뉴스 레이더 v17.14】",
           "우선순위: 프로젝트 공식 홈페이지/블로그 → 기존 주요언론/RSS",
           "자동알람: 게시시각 확인 가능한 36시간 이내 신규 공식뉴스만"]
    for symbol in ("WLD","KAIA"):
        parts.append("\n【"+symbol+"】"); rows=official_project_news(symbol,8)
        if not rows: parts.append("• 공식 소스 신규 항목 확인 실패/없음"); continue
        for it in rows[:4]:
