...
def run_enginetest(cid):
    send("🧪 v10.1 판단엔진 테스트 시작 — 실제 보유 상태/저장값은 변경하지 않습니다.",cid)
    ...
    header="✅ v10.1 판단엔진 전체 통과" if all_ok else "❌ v10.1 판단엔진 일부 실패"
    send(header+"\n\n"+"\n\n".join(lines),cid)
    ...
    while True:
        ...
        for u in r.get("result",[]):
            ...
            if text.startswith("/start") or text.lower()=="start":
                send("✅ Jaina Coin Monitor v10 연결 완료\n/status 현재상태\n/news 최신 크립토·KAIA·WLD 뉴스\n/market BTC 시장요약\n/test 알림테스트\n\n뉴스 브리핑은 2시간마다 자동 발송\n※ 자동주문 없음",cid)
...
<div class="w"><h2>자이나 코인원 감시봇 v10</h2><small>WLD · KAIA / 준비·실행·황금구간 / 자동주문 없음</small><div id="x"></div></div>
<script>
