...
def strategy(symbol, tick):
    ...
    with LOCK:
        ...
        # 3) 조정 재매수: 익절금이 있을 때만 검토
            ...
            rebuy_note="단, 익절금이 있고 급락이 멈추는지 확인"
            reason=f"고점대비 {price_dd:.1f}%"

        elif price_dd<=-12:
            signal="🔵 2차 재매수 후보"
            protect_action="익절금의 30% 이내 재매수 검토"
            rebuy_note="단, 1분 하락세 완화 여부 확인"
            reason=f"고점대비 {price_dd:.1f}%"

        elif price_dd<=-7:
            signal="🔵 1차 재매수 후보"
            protect_action="익절금의 20% 이내 재매수 검토"
            rebuy_note="단, 추가 하락 대비 현금 여유 유지"
            reason=f"고점대비 {price_dd:.1f}%"
...
def evaluate_test_case(gain, price_dd, profit_dd, score):
    """
    실제 strategy()와 같은 우선순위로 핵심 신호만 검증하는 순수 테스트 함수.
    실제 STATE/최고가/평가수익 저장값은 변경하지 않습니다.
    """
    if score >= 5 and price_dd > -3:
        return "🔴 급등 과열 · 2차 익절 후보"
    elif score >= 3 and price_dd > -3:
        return "🟠 급등 강함 · 1차 익절 준비"
    elif gain >= 15 and profit_dd <= -30:
        return "🛑 수익보호 강경 · 3차 익절 검토"
    elif gain >= 15 and profit_dd <= -20:
        return "🔴 수익보호 2차 익절 후보"
    elif gain >= 15 and profit_dd <= -10:
        return "🟠 수익보호 1차 익절 준비"
    elif price_dd <= -25:
        return "🛑 급락 점검 · 재매수 보류"
    elif price_dd <= -18:
        return "🔵 3차 재매수 후보"
...
def engine_test_results():
    cases = [
        ("급등 1차", 25, -1.0, -2.0, 3, "🟠 급등 강함 · 1차 익절 준비"),
        ("급등 2차", 30, -1.0, -3.0, 5, "🔴 급등 과열 · 2차 익절 후보"),
        ("수익보호 -10%", 25, -4.0, -10.0, 0, "🟠 수익보호 1차 익절 준비"),
        ("수익보호 -20%", 25, -4.0, -20.0, 0, "🔴 수익보호 2차 익절 후보"),
        ("수익보호 -30%", 25, -4.0, -30.0, 0, "🛑 수익보호 강경 · 3차 익절 검토"),
        ("재매수 -7%", 10, -7.0, -5.0, 0, "🔵 1차 재매수 후보"),
        ("재매수 -12%", 10, -12.0, -5.0, 0, "🔵 2차 재매수 후보"),
        ("재매수 -18%", 10, -18.0, -5.0, 0, "🔵 3차 재매수 후보"),
        ("급락 -25%", 10, -25.0, -5.0, 0, "🛑 급락 점검 · 재매수 보류"),
        ("평시", 10, -2.0, -2.0, 0, "⚪ 홀딩 / 관찰"),
    ]

    results = []
    all_ok = True
    for name, gain, price_dd, profit_dd, score, expected in cases:
        actual = evaluate_test_case(gain, price_dd, profit_dd, score)
        ok = (actual == expected)
...
def run_enginetest(cid):
    send("🧪 판단엔진 테스트 시작 — 실제 보유 상태/저장값은 변경하지 않습니다.", cid)
    ...
    header = "✅ 판단엔진 전체 통과" if all_ok else "❌ 판단엔진 일부 실패"
    send(header + "\n\n" + "\n\n".join(lines), cid)
    ...
    while True:
        ...
        for u in r.get("result",[]):
            ...
            if text.startswith("/start") or text.lower()=="start":
                send("✅ Jaina Coin Monitor v9.3 연결 완료\n/status 현재상태\n/news 최신 크립토·KAIA·WLD 뉴스\n/market BTC 시장요약\n/test 알림테스트\n\n뉴스 브리핑은 2시간마다 자동 발송\n※ 자동주문 없음",cid)
...
<div class="w"><h2>자이나 코인원 감시봇 v9.3</h2><small>WLD · KAIA / 전략 신호 + 뉴스 / 자동주문 없음</small><div id="x"></div></div>
<script>
