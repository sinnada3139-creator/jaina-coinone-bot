...
BOT_VERSION = "15.8"
CAUSE_ENGINE = "RSS-TRANSPORT-DIAG-v8"
...
def bing_news_rss(query, limit=6):
    ...
    try:
        ...
        r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.8"},timeout=(2.5,5.0))
        r.raise_for_status()
...
def direct_crypto_feeds(limit=10):
    ...
    def fetch_one(source,url):
        key="feed::"+source
        try:
            r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.8"},timeout=(1.8,3.2))
            r.raise_for_status()
...
def direct_macro_feeds(limit=12):
    ...
    def fetch_one(source,url):
        ...
        t0=time.time()
        try:
            r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.8"},timeout=(2.5,5.0),allow_redirects=True)
...
def google_news_rss(query, limit=4, priority=False):
    ...
    try:
        r = SESSION.get(url, headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.8"}, timeout=(2.5,4.5))
        r.raise_for_status()
...
def market_cause_analysis_text(force_direction=None):
    ...
    parts=[f"🌐 【현재 시장 {label} 원인 분석 v15.8】",f"🧠 원인엔진 {CAUSE_ENGINE}",f"BTC {btcprice:,.0f}원 · 1분 {btc1:+.2f}% · 5분 {btc5:+.2f}% · 당일 기준 {btc24:+.2f}%",""]
    ...
    if not selected:
        ...
        parts += [f"🧪 v15.8 진단: GEO {dc['GEO']} · OIL {dc['OIL']} · RATE {dc['RATE']} · RISK {dc['RISK']}"]
        parts += dr
        parts.append("")
