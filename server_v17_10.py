...
def rapid_cause_report_text():
    ...
    _refresh_confirmed_policy_events()
    snap=snapshot(); parts=["🔎 【자이나 현재 급변 원인 레이더 v17.10】"]
    for sym in ("WLD","KAIA"):
        d=snap.get(sym)
        if not d: continue
    ...
    if event_hit:
        ...
        parts.append("\n🔎 【보조 시장 원인 교차검증】\n" + market_cause_analysis_text(forced_dir))
    else:
        parts.append("\n" + market_cause_analysis_text(forced_dir))
    return "\n".join(parts)
...
def clarity_radar_text():
    parts=['🏛️ 【CLARITY Act 전용 레이더 v17.10】','H.R. 3633 · 미 상원 시장구조 법안 집중감시','공식 상원자료 → Reuters/주요언론 → 다중 RSS 교차확인']
    rows=clarity_radar_rows(8)
    if not rows:
