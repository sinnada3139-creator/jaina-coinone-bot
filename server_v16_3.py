...
def catalyst_score_item(item):
    ...
    # v16.3 WLD AI/AGI thematic bridge: score major AI-agent/human-identity news as
    # an indirect catalyst candidate. It is never treated as proof of direct WLD causality.
    ai_core=any(k in text for k in ('openai','chatgpt','sam altman','agi','artificial general intelligence','ai agent','ai agents'))
    ai_impact=any(k in text for k in ('launch','release','released','upgrade','breakthrough','capability','agent','agents','autonomous','identity','human','bot'))
    bridge_bonus=4 if ai_core and ai_impact else 0
...
def catalyst_search(symbol):
    if symbol=="WLD":
        positive_queries=[
            ...
            'OpenAI ChatGPT AGI artificial general intelligence breakthrough model launch when:7d',
...
def bing_news_rss(query, limit=6):
    ...
    try:
        ...
        r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.9"},timeout=(2.5,5.0))
        r.raise_for_status()
...
def direct_crypto_feeds(limit=10):
    ...
    def fetch_one(source,url):
        key="feed::"+source
        try:
            r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.9"},timeout=(1.8,3.2))
            r.raise_for_status()
...
def direct_macro_feeds(limit=12):
    ...
    def fetch_one(source,url):
        ...
        t0=time.time()
        try:
            r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.9"},timeout=(2.5,5.0),allow_redirects=True)
...
def google_news_rss(query, limit=4, priority=False):
    ...
    try:
        r = SESSION.get(url, headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.9"}, timeout=(2.5,4.5))
        r.raise_for_status()
...
LEADING_CATALYST_QUERIES = {
    "WLD": [
        ...
        'OpenAI ChatGPT AGI artificial general intelligence breakthrough model launch when:2d',
