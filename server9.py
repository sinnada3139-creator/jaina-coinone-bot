...
def strategy(symbol, tick):
    ...
    with LOCK:
        ...
        # peak / 최고수익 값은 persistence_loop가 30초마다 저장
        ...
        # 최고 평가수익 보호: 수익권이 충분히 커진 후 되돌림이 커질 때 단계적 익절 제안
        if gain>=15 and peak_profit>0 and profit_dd<=-30:
    ...
    while True:
        ...
        for u in r.get("result",[]):
            ...
            if text.startswith("/start") or text.lower()=="start":
                send("✅ Jaina Coin Monitor v8.1 연결 완료\n/status 현재상태\n/news 최신 크립토·KAIA·WLD 뉴스\n/market BTC 시장요약\n/test 알림테스트\n\n뉴스 브리핑은 2시간마다 자동 발송\n※ 자동주문 없음",cid)
...
<div class="w"><h2>자이나 코인원 감시봇 v8.1</h2><small>WLD · KAIA / 전략 신호 + 뉴스 / 자동주문 없음</small><div id="x"></div></div>
<script>
