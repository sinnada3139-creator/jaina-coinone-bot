...
# ---------- NEWS / MARKET ----------
# v16.2: WLD AI/AGI -> human verification -> World ID thematic bridge
...
CATALYST_WORDS = [
    ...
    "openai","chatgpt","agi","artificial general intelligence","ai agent","ai agents",
    "proof of human","proof of personhood","human verification","digital identity",
    "인공지능","범용인공지능","AI 에이전트","인간 인증","인간 증명","디지털 신원"
]
...
def catalyst_search(symbol):
    if symbol=="WLD":
        positive_queries=[
            ...
            'OpenAI ChatGPT AGI \"AI agent\" \"human verification\" \"proof of human\" \"digital identity\" when:7d',
            'OpenAI ChatGPT AGI Worldcoin \"World ID\" Sam Altman when:7d',
        ]
        ...
        themes=[
            ...
            "거래소·기관·기업 파트너십",
            "토큰 언락/공급 증가 일정",
            "OpenAI·ChatGPT·AGI·AI Agent 확산 → 인간증명/World ID 수요 연결"
        ]
    else:
        positive_queries=[
...
LEADING_CATALYST_QUERIES = {
    "WLD": [
        ...
        'OpenAI OR ChatGPT OR AGI OR \"artificial general intelligence\" OR \"Sam Altman\" OR \"AI agent\" OR \"AI agents\" when:2d',
        '(OpenAI OR ChatGPT OR AGI OR \"AI agent\") (\"World ID\" OR Worldcoin OR \"proof of human\" OR \"human verification\" OR \"proof of personhood\") when:7d',
    ],
    "KAIA": [
...
LEADING_INDIRECT = {
    "WLD": ('openai','chatgpt','sam altman','agi','artificial general intelligence','ai agent','ai agents','proof of human','proof-of-human','proof of personhood','human verification','digital identity','인공지능','범용인공지능','ai 에이전트','인간 인증','인간 증명','디지털 신원'),
    "KAIA": ('stablecoin','스테이블코인','krw','원화','bank','은행','kakao','카카오','line','라인','rwa','payment','결제'),
}
LEADING_AI_IMPACT = ('agi','artificial general intelligence','breakthrough','frontier model','new model','model release','launch','upgrade','agent','agents','reasoning','autonomous','computer use','범용인공지능','신모델','모델 공개','출시','업그레이드','에이전트','자율')
...
def _leading_grade(symbol,title):
    ...
    if direct and future:
        return ('A','직접 예정 이벤트')
    ai_impact=(symbol=='WLD' and indirect and any(k in low for k in LEADING_AI_IMPACT))
    if indirect and (positive or future or ai_impact):
        return ('A','AI/AGI→인간증명·World ID 간접 선행호재 후보' if ai_impact else '간접 선행호재 후보')
    return None
