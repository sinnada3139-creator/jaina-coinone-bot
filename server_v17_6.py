...
def _clarity_stage(title):
    low=(title or '').lower()
    # Accept exact bill aliases, plus market-structure headlines that clearly carry crypto/Senate context.
    ...
    # Outcome terms outrank preview language. Avoid treating a mere "60 votes needed" preview as a result.
    result_terms=('passes','passed','advances','advanced','cloture invoked','approved','fails','failed','rejected','blocked','stalls','stalled','vote result','표결 결과','가결','부결','통과','무산')
    if any(k in low for k in result_terms): return ('RESULT','🔴')
    ...
    if any(k in low for k in ('vote','cloture','motion to proceed','scheduled','hearing','60 votes','표결','예정','청문')): return ('PREVIEW','🟠')
    return ('NEWS','⚪')
...
def _clarity_official_rows():
    ...
    for source,url in CLARITY_OFFICIAL_SOURCES:
        try:
            r=SESSION.get(url,timeout=(3.0,8.0),headers={'User-Agent':'Mozilla/5.0 JainaCoinMonitor/17.6'})
            if r.status_code != 200: continue
            txt=html.unescape(re.sub(r'<[^>]+>',' ',r.text)); txt=re.sub(r'\s+',' ',txt)
            low=txt.lower(); positions=[]
            for a in ('h.r.3633','h.r. 3633','digital asset market clarity act','clarity act','regulatory system for digital commodities'):
                pos=low.find(a)
                if pos>=0: positions.append(pos)
            for pos in positions[:2]:
                snippet=txt[max(0,pos-220):pos+700].strip()
                stage,level=_clarity_stage(snippet) or ('NEWS','⚪')
                out.append((stage,level,0.0,{'title':snippet[:520],'source':source,'link':url,'official':True}))
        except Exception as e:
    ...
    for q in CLARITY_QUERIES:
        try:
            ...
            for it in items:
                ...
                age=_news_age_hours(it)
                if age > 96: continue
                key=re.sub(r'\W+',' ',title.lower())[:180]
...
def clarity_radar_text():
    parts=['🏛️ 【CLARITY Act 전용 레이더 v17.6】','H.R. 3633 · 미 상원 시장구조 법안 집중감시','공식 상원자료 → Reuters/주요언론 → 다중 RSS 교차확인']
    rows=clarity_radar_rows(10)
    if not rows:
        if NEWS_HEALTH.get('clarity_paths',0)==0:
            parts.append('⚠️ 뉴스 수집망 응답 실패/지연 — "신규 기사 없음"으로 단정하지 않음')
        else:
            parts.append('• 최근 96시간 수집 결과에서 CLARITY 관련 신규 항목을 확인하지 못함')
    else:
    ...
    parts += ['※ cloture/motion-to-proceed는 최종 법안 통과와 구분합니다.','※ 가격 움직임 전이라도 RESULT/CHANGE/UPDATE는 중요알람 대상 · 자동주문 없음']
    return '\n'.join(parts)
...
def clarity_radar_loop():
    while True:
        try:
            if CHAT_ID:
                now=time.time()
                for stage,level,age,it in clarity_radar_rows(10):
