...
def bing_news_rss(query, limit=6):
    ...
    try:
        ...
        r=SESSION.get(url,headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.1"},timeout=(2.5,5.0))
        r.raise_for_status()
...
def google_news_rss(query, limit=4, priority=False):
    ...
    try:
        r = SESSION.get(url, headers={"User-Agent":"Mozilla/5.0 JainaCoinMonitor/15.1"}, timeout=(2.5,4.5))
        r.raise_for_status()
...
def _macro_news(direction="DOWN"):
    ...
    if len(fresh0) < 12:
        urgent = stage1[-4:] if direction == "DOWN" else stage1[-1:] + stage1[:1]
...
def _is_cross_asset_catalyst(title, cat):
    """v15.1: 원유 상승/전쟁 격화처럼 '제목 방향'은 상승이어도 BTC에는 악재인 교차자산 촉매를 식별."""
    low=(title or "").lower()
    if cat == "에너지·유가":
...
def market_cause_analysis_text(force_direction=None):
    ...
    for it in macro_items:
        ...
        catalyst_bonus=12 if cross_asset else 0
    ...
    parts=[f"🌐 【현재 시장 {label} 원인 분석 v15.1】",f"BTC {btcprice:,.0f}원 · 1분 {btc1:+.2f}% · 5분 {btc5:+.2f}% · 당일 기준 {btc24:+.2f}%",""]
