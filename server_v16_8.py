...
def v165_agi_candidates():
    rows,seen=[],set()
    diag={'queries':len(AGI_WLD_QUERY_PACK),'collected':0,'clean':0,'scored':0,'sources':{}}
    ...
    for q,items,trace in results:
        for name,n,status in trace:
            ...
            d['attempts']+=1; d['items']+=n; d['last']=status
...
def v165_diag_text():
    ...
    if not rows:
        lines += ['', '❌ AGI/OpenAI 후보 0건', '→ RSS 수집 자체 또는 네트워크 응답을 점검해야 합니다.']
    else:
        lines += ['', '🔎 상위 후보']
